...
m_bFiltChrom = m_RegExprs.Accept(pszChrom);
return(m_bFiltChrom);
}
...
if(m_pCurLocChrom != NULL &&
   !stricmp(m_pCurLocChrom->szChrom,pszChrom))
	...
	if(m_NumChroms == 0 || Idx ==  m_NumChroms)
		...
		if(pFeature == NULL)
			{
			gDiagnostics.DiagOut(eDLFatal,gszProcName,"AddFeature: Memory allocation of %zd bytes - %s",(int64_t)memreq,strerror(errno));
			Reset();
			return(eBSFerrMem);
			}
#else
		...
		if(pFeature == MAP_FAILED)
			{
			gDiagnostics.DiagOut(eDLFatal,gszProcName,"AddFeature: Memory allocation of %zd bytes through mmap()  failed - %s",(int64_t)memreq,strerror(errno));
			Reset();
			return(eBSFerrMem);
			}
#endif
