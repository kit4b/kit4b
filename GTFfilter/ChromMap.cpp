...
if(m_pChromMaps == NULL)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadChromMap: Memory allocation of %I64d bytes - %s",(int64_t)memreq,strerror(errno));
	Reset();
	return(eBSFerrMem);
	}
#else
...
if(m_pChromMaps == MAP_FAILED)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadChromMap::Load: Memory allocation of %I64d bytes through mmap()  failed - %s",(int64_t)memreq,strerror(errno));
	m_pChromMaps = NULL;
	Reset();
	return(eBSFerrMem);
	}
