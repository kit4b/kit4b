...
if (!argerrors)
	...
	gDiagnostics.DiagOut(eDLInfo,gszProcName,"Process %s Version %s starting",gszProcName,kit4bversion);
...
if((pSAMStream = fopen(pszSAMFile,"r"))==NULL)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"ParseSAMFileElements: Unable to fopen SAM/BAM format file %s error: %s",pszSAMFile,strerror(errno));
	return(eBSFerrOpnFile);
	}
