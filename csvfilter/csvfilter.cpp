...
// prealloc memory 
	...
	if(pParams->pElChrBuff == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"FilterCSV: Memory allocation of %zd bytes - %s",(int64_t)cElChrAllocChunk,strerror(errno));
		Cleanup(pParams);
		return(eBSFerrMem);
		}
#else
// gnu malloc is still in the 32bit world and can't handle more than 2GB allocations
	...
	if(pParams->pElChrBuff == MAP_FAILED)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"FilterCSV: Memory allocation of %zd bytes through mmap()  failed - %s",(int64_t)cElChrAllocChunk,strerror(errno));
	...
	if(pParams->pElLocs == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"FilterCSV: Memory allocation of %zd bytes - %s",(int64_t)cElLociAllocChunk * sizeof(tsCSVEl),strerror(errno));
		Cleanup(pParams);
		return(eBSFerrMem);
		}
#else
// gnu malloc is still in the 32bit world and can't handle more than 2GB allocations
	...
	if(pParams->pElLocs == MAP_FAILED)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"FilterCSV: Memory allocation of %zd bytes through mmap()  failed - %s",(int64_t)cElLociAllocChunk * sizeof(tsCSVEl),strerror(errno));
