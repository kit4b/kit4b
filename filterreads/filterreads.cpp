...
if(m_pAlignHits == NULL)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"InitialMemAlloc: Memory allocation of %zd bytes - %s",(int64_t)memreq,strerror(errno));
	Reset();
	return(eBSFerrMem);
	}
#else
...
if(m_pAlignHits == MAP_FAILED)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"InitialMemAlloc: Memory allocation of %zd bytes through mmap()  failed - %s",(int64_t)memreq,strerror(errno));
	m_pAlignHits = NULL;
	Reset();
	return(eBSFerrMem);
	}
