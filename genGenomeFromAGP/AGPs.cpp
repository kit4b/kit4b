...
if(m_pAGPentries == NULL)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"ProcessAGP: Memory allocation of %zd bytes - %s",(int64_t)memreq,strerror(errno));
	Reset();
	return(eBSFerrMem);
	}
#else
...
if(m_pAGPentries == MAP_FAILED)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"ProcessAGP: Memory allocation of %zd bytes through mmap()  failed - %s",(int64_t)memreq,strerror(errno));
	m_pAGPentries = NULL;
	Reset();
	return(eBSFerrMem);
	}
