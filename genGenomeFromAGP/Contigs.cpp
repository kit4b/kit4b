...
if(m_pContigEntries == NULL)
	...
	if(m_pContigEntries == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadContigFile: Memory allocation of %zd bytes for contig entries - %s",(int64_t)memreq,strerror(errno));
		Reset();
		return(eBSFerrMem);
		}
#else
	...
	if(m_pContigEntries == MAP_FAILED)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadContigFile: Memory allocation of %zd bytes through mmap() for contig entries failed - %s",(int64_t)memreq,strerror(errno));
		m_pContigEntries = NULL;
		Reset();
		return(eBSFerrMem);
		}
...
if(m_pContigSeqs == NULL)
	...
	if(m_pContigSeqs == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadContigFile: Memory allocation of %zd bytes for contig sequences - %s",(int64_t)memreq,strerror(errno));
		Reset();
		return(eBSFerrMem);
		}
#else
	...
	if(m_pContigSeqs == MAP_FAILED)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadContigFile: Memory allocation of %zd bytes through mmap() for contig sequences failed - %s",(int64_t)memreq,strerror(errno));
		m_pContigSeqs = NULL;
		Reset();
		return(eBSFerrMem);
		}
