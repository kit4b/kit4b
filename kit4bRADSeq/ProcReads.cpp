...
if(m_pP1Seqs2Assemb == NULL)		// will be NULL first time
	...
	if(m_pP1Seqs2Assemb == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadRawReads: Concatenated sequences P1 memory allocation of %zd bytes - %s",(int64_t)m_AllocMemP1Seqs2Assemb,strerror(errno));
		return(eBSFerrMem);
		}
#else
	...
	if(m_pP1Seqs2Assemb == MAP_FAILED)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadRawReads: Concatenated sequences P1 memory of %zd bytes through mmap()  failed - %s",(int64_t)m_AllocMemP1Seqs2Assemb,strerror(errno));
...
#ifdef _WIN32
		pDstSeq = (uint8_t *) realloc(m_pP1Seqs2Assemb,memreq);
#else
		pDstSeq = (uint8_t *)mremap(m_pP1Seqs2Assemb,m_AllocMemP1Seqs2Assemb,memreq,MREMAP_MAYMOVE);
		if(pDstSeq == MAP_FAILED)
			pDstSeq = NULL;
#endif
		if(pDstSeq == NULL)
			{
			gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadRawReads: P1 Memory re-allocation to %zd bytes - %s",memreq,strerror(errno));
			return(eBSFerrMem);
			}
...
if(P2ReqAllocSize > 0)
	...
	if(m_pP2Seqs2Assemb == NULL)		// will be NULL first time
		...
		if(m_pP2Seqs2Assemb == NULL)
			{
			gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadRawReads: Concatenated P2 sequences memory allocation of %zd bytes - %s",(int64_t)m_AllocMemP2Seqs2Assemb,strerror(errno));
			return(eBSFerrMem);
			}
#else
		// gnu malloc is still in the 32bit world and can't handle more than 2GB allocations
		...
		if(m_pP2Seqs2Assemb == MAP_FAILED)
			{
			gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadRawReads: Concatenated P2 sequences memory of %zd bytes through mmap()  failed - %s",(int64_t)m_AllocMemP2Seqs2Assemb,strerror(errno));
...
#ifdef _WIN32
			pDstSeq = (uint8_t *) realloc(m_pP2Seqs2Assemb,memreq);
#else
			pDstSeq = (uint8_t *)mremap(m_pP2Seqs2Assemb,m_AllocMemP2Seqs2Assemb,memreq,MREMAP_MAYMOVE);
			if(pDstSeq == MAP_FAILED)
				pDstSeq = NULL;
#endif
			if(pDstSeq == NULL)
				{
				gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadRawReads: Memory P2 re-allocation to %zd bytes - %s",memreq,strerror(errno));
				return(eBSFerrMem);
				}
...
if(m_pSeqStarts == NULL)
	...
	if(m_pSeqStarts == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadRawReads: map sequence starts memory allocation of %zd bytes - %s",(int64_t)m_AllocMemSeqStarts,strerror(errno));
		return(eBSFerrMem);
		}
#else
	// gnu malloc is still in the 32bit world and can't handle more than 2GB allocations
	...
	if(m_pSeqStarts == MAP_FAILED)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadRawReads: map sequence starts memory allocation of %zd bytes through mmap()  failed - %s",(int64_t)m_AllocMemSeqStarts,strerror(errno));
		m_pSeqStarts = NULL;
		return(eBSFerrMem);
		}
	#endif
...
CFasta P1Fasta;
int P1DescrLen;
uint8_t szP1DescrBuff[1024];
etSeqBase *pP1Seq;
uint8_t *pP1RawReadsBuff;
int P1ReadLen;
bool bP1IsFastq;
...
CFasta P2Fasta;
int P2DescrLen;
uint8_t szP2DescrBuff[1024];
etSeqBase *pP2Seq;
uint8_t *pP2RawReadsBuff;
int P2ReadLen;
bool bP2IsFastq;
...
if((pP1RawReadsBuff = new uint8_t [cAllocRawSeqLen]) == NULL)
	{
...
if(bIsPairedEndProc)
	{
	if((pP2RawReadsBuff = new uint8_t [cAllocRawSeqLen]) == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"Unable to allocate memory for raw reads buffering...");
		delete pP1RawReadsBuff;
		P1Fasta.Close();
...
if(MemWorkSetBytes > m_CurMaxMemWorkSetBytes)
	...
	if(!SetMaxMemWorkSetSize(MemWorkSetBytes  * 2))
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadReads: Length of loaded concatenated sequences exceeds limit of %zd bytes",cMaxConcatSeqLen);
		P1Fasta.Close();
		delete pP1RawReadsBuff;
		if(bIsPairedEndProc)
			{
...
while((Rslt = (teBSFrsltCodes)(P1ReadLen = P1Fasta.ReadSequence(pP1RawReadsBuff,cAllocRawSeqLen,true,false))) > eBSFSuccess)
	...
	if(P1ReadLen == eBSFFastaDescr)		// just read a descriptor line which would be as expected for multifasta or fastq
		...
		if(m_P1Seqs2AssembLen > cMaxConcatSeqLen || m_P2Seqs2AssembLen > cMaxConcatSeqLen)			// hit limit?
			{
			gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadReads: Length of loaded concatenated %s sequences exceeds limit of %zd bytes",
...
gDiagnostics.DiagOut(eDLInfo,gszProcName,"GenRdsSfx: Now generating suffix array with %zd index elements size %d bytes..",MaxSuffixEls,ElSize);
...
if(m_pSuffixArray == NULL || m_AllocMemSfx == 0)
	...
	if(m_pSuffixArray == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"GenRdsSfx: Suffix array memory allocation of %zd bytes - %s",(int64_t)m_AllocMemSfx,strerror(errno));
		m_AllocMemSfx = 0;
		Reset(false);
		return(eBSFerrMem);
		}
	...
	if(m_pSuffixArray == MAP_FAILED)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"GenRdsSfx: Suffix array memory allocation of %zd bytes through mmap()  failed - %s",(int64_t)m_AllocMemSfx,strerror(errno));
		m_pSuffixArray = NULL;
		m_AllocMemSfx = 0;
		Reset(false);
		return(eBSFerrMem);
	...
	if(ReqAllocMem >  (size_t)m_AllocMemSfx || ((ReqAllocMem * 10) <  ((size_t)m_AllocMemSfx * 11)))
		...
		if(pTmp == NULL)
			{
			gDiagnostics.DiagOut(eDLFatal,gszProcName,"GenRdsSfx: Memory re-allocation to %zd bytes - %s",ReqAllocMem,strerror(errno));
			return(eBSFerrMem);
			}
		m_AllocMemSfx = ReqAllocMem;
		m_pSuffixArray = pTmp;
...
teBSFrsltCodes
CStackSeqs::ChunkedRead(int64_t RdOfs,uint8_t *pData,int64_t RdLen)
{
...
// Seeks to specified 64bit file offset and reads from disk as chunks of no more than INT_MAX/32
teBSFrsltCodes
CStackSeqs::ChunkedRead(int hFile,char *pszFile,int64_t RdOfs,uint8_t *pData,int64_t RdLen)
{
int BlockLen;
if(_lseeki64(hFile,RdOfs,SEEK_SET) != RdOfs)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"Unable to seek to %zd on file - error %s",RdOfs,pszFile,strerror(errno));
