...
if(m_pAlignmentSummaries == NULL)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"CreateDatabase: Memory allocation of %zd bytes for alignment summary instances failed - %s",(int64_t)memreq,strerror(errno));
	sqlite3_close_v2(m_pDB);
	sqlite3_shutdown();
	m_pDB = NULL;
	return(NULL);
...
if((m_allocAlignmentSummariesSize - m_UsedAlignmentSummariesSize) < (2 * (sizeof(tsAlignSummary) + (size_t)(cMaxPSLSeqNameLen))))
	...
	if(m_pAlignmentSummaries == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"AddAlignSummary: Memory reallocation to %zd bytes for alignment summary instances failed - %s",(int64_t)memreq,strerror(errno));
		return(eBSFerrMem);
		}
	m_allocAlignmentSummariesSize = memreq;
	}
