...
if(m_pContaminants == NULL)
	...
	if (m_pContaminants == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "AddFlankContam: unable to allocate for Contaminant memory %zu", AllocMem);
		Reset();
		return(eBSFerrMem);
		}
#else
...
if((m_NumFlankContaminates + 1) > m_AllocContaminants)		// time to realloc some more Contaminants? 
	...
	if (pRealloc == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "AddFlankContam: Memory re-allocation to %zd bytes - %s", ReallocSize, strerror(errno));
		return(eBSFerrMem);
		}
...
if(m_pContamSeqNodes == NULL)
	...
	if (m_pContamSeqNodes == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "IndexContamSeq: unable to allocate for ContamSeqNodes memory %zu", AllocMem);
		Reset();
		return(eBSFerrMem);
		}
#else
...
if(m_NumContamSeqNodes + cMaxContaminantLen + 10 >= m_AllocdContamSeqNodes)
	...
	if (pRealloc == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "IndexContamSeq: Memory re-allocation to %zd bytes - %s", AllocMem, strerror(errno));
		return(eBSFerrMem);
		}
