...
char szTargAssemblyName[cMaxDatasetSpeciesChrom+1]; // targeted assembly against which SNPs in a 'kit4b snpmarkers' generated file were called
int NumCultivarNames;
char szCultivarNames[100][cMaxDatasetSpeciesChrom+1];		// name of this cultivar, can be at most 100 cultivars in a 'kit4b snpmarkers' generated file
int CultEls;
...
while((Rslt=pCSV->NextLine()) > 0)	// onto next line containing fields
	...
	if(!m_NumElsParsed && (NumFields > 7 && pCSV->IsLikelyHeaderLine())) // expecting at least 8 fields so only then check for header line
		...
		if(NumFields < 12)
			continue;
				// at least 12 fields then parse header assuming it is a 'kit4b snpmarkers' format
	...
	if(AutoFormat == 0 && CSVFormat == eCSVFdefault)							// default layout and not checked for SNP call formats?
		...
		// if 'kit4b snpmarkers' generated then there will be (4 + ({8 or 9}) * numcultivars) fields, {8} if pre-3.0.6 or {9} if post-3.0.6
        ...
        // to identify as a 'kit4b snpmarkers' then check if num fields as expected for this format and that field 4 is not text chrom but is integer 1..numcultivars
	...
	switch(CSVFormat) {
		case eCSVFdefault:							// default layout
			if(AutoFormat == 2)			// simple SNP loci with optional variant bases
				...
				{
				if(AutoFormat == 3 ||				// 'kit4b snpmarkers' generated with 9 fields per cultivar
					AutoFormat == 4)				// 'kit4b snpmarkers' generated with 8 fields per cultivar
					{
					SrcID += 1;
					if(MinLength > 1)
						{
...
// If de Novo then expecting user to have aligned against a psuedo genome in which contigs were concatenated with spacer N's such that there is a single chromosome
int				// returns number of elements parsed
CHyperEls::ParseSAMFileElements(char *pszFile,	  // file (in SAM/BAM format) containing element loci
							 int MinLength,  // slough any elements which are less MinLength length
							 int MaxLength,  // slough any elements which are more than MaxLength length
