...
if(m_SAMFileType >= eSFTBAM_BAI && (size_t)(uint64_t)SeqLen > cMaxCSIRefSeqLen)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"AddREfSeq: Reference sequence '%s':'%s' of length %d is longer than max allowed %s index offset of %zd",
...
if(m_SAMFileType >= eSFTBAM_BAI)
	...
	if(m_SAMFileType == eSFTBAM_BAI && m_MaxRefSeqLen >= cMaxSAIRefSeqLen)
		{
		gDiagnostics.DiagOut(eDLInfo,gszProcName,"StartAlignments: Forced generation of CSI instead of SAI index file as alignments to sequence lengths (max %zd) more than SAI 512Mbp limit",m_MaxRefSeqLen);
		free(m_pChunkBins);
		m_pChunkBins = NULL;
...
if(m_SAMFileType >= eSFTBAM_BAI)
	...
	if((m_p16KOfsVirtAddrs = (uint64_t *)malloc(m_Alloc16KOfsVirtAddrsSize))==NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"StartAlignments: unable to alloc %zd memory for BAI or CSI 16kb linear virtual offsets",m_Alloc16KOfsVirtAddrsSize);
		Reset();
		return(eBSFerrMem);
		}
...
if(End >= m_MaxIdxRefSeqLen)		// exceeding the limit should have been picked up earlier but better safe than sorry
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"AddAlignment: Unable to accept alignment ending at %u in %s index extending past %zdbp'",
...
if(m_SAMFileType == eSFTSAM || m_SAMFileType == eSFTSAMgz)
	...
	if(pBAMalign->pos >= 0 && m_SAMFileType >= eSFTBAM_BAI && pBAMalign->end >= m_MaxIdxRefSeqLen)		
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"AddAlignment: Unable to accept alignment ending at %d in %s index extending past %zdbp",
