...
if(m_pChromSeqs == NULL)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadFasta: Memory allocation of %zd bytes failed",(int64_t)m_AllocdChromSize);
	m_AllocdChromSize = 0;
	Reset(false);
	return(eBSFerrMem);
	}
...
if(m_pChromSeqs == MAP_FAILED)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadFasta: Memory allocation of %zd bytes through mmap()  failed",(int64_t)m_AllocdChromSize,strerror(errno));
	m_pChromSeqs = NULL;
	m_AllocdChromSize = 0;
	Reset(false);
	return(eBSFerrMem);
...
if(m_pGenomeSeq == NULL)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadFasta: Memory allocation of %zd bytes failed",(int64_t)m_AllocdGenomeMem);
	Reset(false);
	return(eBSFerrMem);
	}
#else
...
if(m_pGenomeSeq == MAP_FAILED)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadFasta: Memory allocation of %zd bytes through mmap()  failed",(int64_t)m_AllocdGenomeMem,strerror(errno));
	m_pGenomeSeq = NULL;
	Reset(false);
	return(eBSFerrMem);
	}
...
while((Rslt = SeqLen = Fasta.ReadSequence(pSeqBuff,cSRMaxAllocBuffChunk-1,true,false)) > eBSFSuccess)
	...
	if(bFirstEntry || SeqLen == eBSFFastaDescr)		// just read a descriptor line
		...
		if(ChromID == m_AllocdChromSeqs)
			...
			if(pTmp == NULL)
				{
				gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadFasta: Memory re-allocation to %zd bytes - %s",(int64_t)(memreq),strerror(errno));
				delete pSeqBuff;
				return(eBSFerrMem);
				}
			m_pChromSeqs = pTmp;
...
if(m_pChromSeqs == NULL)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadAssembly: Memory allocation of %zd bytes failed",(int64_t)m_AllocdChromSize);
	Reset(false);
	return(eBSFerrMem);
	}
#else
...
if(m_pChromSeqs == MAP_FAILED)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadAssembly: Memory allocation of %zd bytes through mmap()  failed",(int64_t)m_AllocdChromSize,strerror(errno));
	m_pChromSeqs = NULL;
	Reset(false);
	return(eBSFerrMem);
	}
...
if(m_pGenomeSeq == NULL)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadAssembly: Memory allocation of %zd bytes failed",(int64_t)m_AllocdGenomeMem);
	Reset(false);
	return(eBSFerrMem);
	}
#else
...
if(m_pGenomeSeq == MAP_FAILED)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadAssembly: Memory allocation of %zd bytes through mmap()  failed",(int64_t)m_AllocdGenomeMem,strerror(errno));
	m_pGenomeSeq = NULL;
	Reset(false);
	return(eBSFerrMem);
	}
...
if(m_pGenomeSeq == NULL)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadTranscriptome: Memory allocation of %zd bytes failed",(int64_t)m_GenomeLen);
	Reset(false);
	return(eBSFerrMem);
	}
#else
...
if(m_pGenomeSeq == MAP_FAILED)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"LoadAssembly: Memory allocation of %zd bytes through mmap()  failed",(int64_t)m_GenomeLen,strerror(errno));
	m_pGenomeSeq = NULL;
	Reset(false);
	return(eBSFerrMem);
	}
...
if(m_pSimReads == NULL)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"Process: Memory allocation of %zd bytes for simulated reads failed",(int64_t)m_AllocdMemReads);
	Reset(false);
	return(eBSFerrMem);
	}
#else
...
if(m_pSimReads == MAP_FAILED)
	{
	gDiagnostics.DiagOut(eDLFatal,gszProcName,"Process: Memory allocation of %zd bytes for simulated reads through mmap()  failed",(int64_t)m_AllocdMemReads,strerror(errno));
	m_pSimReads = NULL;
	Reset(false);
	return(eBSFerrMem);
	}
