...
const int cMaxBatchSize = 5000000;		// max number of defaulted and non-profiled reads to simulate per thread before checkpointing to disk
...
typedef struct TAG_sSRChromSeq {
	...
	uint32_t SeqOfs;			// offset at which this chroms sequence starts
} tsSRChromSeq;
...
typedef struct TAG_sSRWorkerPars {
	...
	bool bDedupe;			// if true then slough any reads exactly matching other reads
...
class CSimReads
{
	...
	uint8_t *m_pGenomeSeq;			// allocated to hold concatenated (separated by eBaseEOSs) chromosome sequences
	...
	int64_t m_AllocdMemReads;			// actual memory allocation size used when allocating m_pSimReads
