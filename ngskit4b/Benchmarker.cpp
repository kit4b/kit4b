...
if((Rslt = LoadGroundTruths(pszSEReads, pszPE2Reads,true)) < 1)
	return(Rslt);
gDiagnostics.DiagOut(eDLInfo, gszProcName, "Total of %u ground truth reads loaded, total sequence length %zd, having potentially %zd ground truth aligning bases", m_NumGroundTruthReads,m_TotGroundTruthReadBases,m_TotNumPotentialAlignBases);
...
gDiagnostics.DiagOut(eDLInfo, gszProcName, "Ground truth reads: %u containing %zd bases of which %zd were potential ground truth bases", m_NumGroundTruthReads, (int64_t)m_TotGroundTruthReadBases, (int64_t)m_TotNumPotentialAlignBases);
...
gDiagnostics.DiagOut(eDLInfo, gszProcName,"Bases loci correct: %zd, misaligned: %zd, unaligned: %zd", m_NumBasesLociCorrect, m_NumBasesLociIncorrect, m_NumBasesLociUnclaimed);
...
#ifdef _WIN32	
	LineBuffIdx += sprintf(&szLineBuffer[LineBuffIdx], "%u,%zd,%zd,", m_NumGroundTruthReads, (int64_t)m_TotGroundTruthReadBases,(int64_t)m_TotNumPotentialAlignBases);
#else
...
#ifdef _WIN32
	LineBuffIdx += sprintf(&szLineBuffer[LineBuffIdx], "%zd,%zd,%zd,", (int64_t)m_NumBasesLociCorrect, (int64_t)m_NumBasesLociIncorrect, (int64_t)m_NumBasesLociUnclaimed);
#else
