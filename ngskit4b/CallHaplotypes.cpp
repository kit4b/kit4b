...
eModeCSH PMode;				// processing mode 
							...
							// 11 filter previously generated scores file by source and reference PBAs names into new filtered scores file
...
int32_t NumIncludeSrcPBAsScores;		// if filtering scores then only retain scores for this number of source PBAs
char* pszIncludeSrcPBAsScores[cMaxFilterPBAsScoresNames]; // names of source PBAs (could be wildcarded)

int32_t NumIncludeRefPBAsScores;		// if filtering scores then only retain scores for this number of reference PBAs
char* pszIncludeRefPBAsScores[cMaxFilterPBAsScoresNames]; // names of reference PBAs (could be wildcarded)
...
struct arg_int *pmode = arg_int0 ("m", "mode", "<int>", "processing mode 0: report imputation haplotype matrix, 1: report both raw and imputation haplotype matrices, 2: additionally generate GWAS allowing visual comparisons, \n\t\t3: allelic haplotype grouping,4: coverage haplotype grouping, 5: post-process haplotype groupings for DGTs, 6: post-process to WIG, 7 rel PBAs vs. ref PBAs, \n\t\t3 8 ref PBAs vs. ref PBAs  allelic association scores,  9 grouping by allelic association scores, \n\t\t3 10 post-processing haplotype groupings for differential group KMers, 11 filter allelic scores by source and reference, 12 filter allelic scores by source and reference with transformation (default 0)");
...
struct arg_int* minunprunedrefs = arg_int0("p", "MinUnprunedRefs", "<int>", "in mode 9: prune whilst at least this number of unpruned references remain (default 1)");
struct arg_int* maxunprunedrefs = arg_int0("P", "MaxUnprunedRefs", "<int>", "in mode 9: attempt to prune untill no more than this number of unpruned references (default 4)");
...
struct arg_str* filtsrcpbascores = arg_strn("r", "filtsrcpbascores", "<string>", 0, cMaxFilterPBAsScoresNames, "proc mode 11 only - retain source BPA scores, wildcards allowed, limit of 100 wildcarded source names supported");
struct arg_str* filtrefpbascores = arg_strn("R", "filtsrefpbascores", "<string>", 0, cMaxFilterPBAsScoresNames, "proc mode 11 only - retain reference BPA scores, wildcards allowed, limit of 100 wildcarded reference names supported");
...
void *argtable[] = { help,version,FileLogLevel,LogFile,
					...
					grphapbinsize,gpphases,mincentclustdist,maxcentclustdist,affinegaplen,maxclustgrps,fndrtrim5,fndrtrim3,progtrim5,progtrim3,wwrlproxwindow,outliersproxwindow,minunprunedrefs,maxunprunedrefs,
					filtsrcpbascores,filtrefpbascores,maskbpafile,chromfile,IncludeChroms,ExcludeChroms,progenyfiles,founderfiles, allelescorefile, outfile,threads,end };
...
if (!argerrors)
	...
	if (FileLogLevel->count && !LogFile->count)
		...
		KMerNoneCoverage = 0;
		NumIncludeSrcPBAsScores = 0;
		NumIncludeRefPBAsScores = 0;
		MinUnprunedRefs = 0;
		MaxUnprunedRefs = 0;
	...
	if (PMode == eMCSHGroupScores || PMode >= eMCSHKFiltScores)
		{
	...
	if(PMode >= eMCSHKFiltScores)
		{
		if (filtrefpbascores->count)
			{
			for (NumIncludeRefPBAsScores = Idx = 0; NumIncludeRefPBAsScores < cMaxFilterPBAsScoresNames && Idx < filtrefpbascores->count; Idx++)
				{
				pszIncludeRefPBAsScores[Idx] = nullptr;
				if (pszIncludeRefPBAsScores[NumIncludeRefPBAsScores] == nullptr)
					pszIncludeRefPBAsScores[NumIncludeRefPBAsScores] = new char[cMaxLenRE+1];
				strncpy(pszIncludeRefPBAsScores[NumIncludeRefPBAsScores], filtrefpbascores->sval[Idx], cMaxLenRE + 1);
				pszIncludeRefPBAsScores[NumIncludeRefPBAsScores][cMaxREs] = '\0';
				CUtility::TrimQuotedWhitespcExtd(pszIncludeRefPBAsScores[NumIncludeRefPBAsScores]);
				if (pszIncludeRefPBAsScores[NumIncludeRefPBAsScores][0] != '\0')
					NumIncludeRefPBAsScores++;
				}
			}
		else
			NumIncludeRefPBAsScores = 0;

		if (filtsrcpbascores->count)
			{
			for (NumIncludeSrcPBAsScores = Idx = 0; NumIncludeSrcPBAsScores < cMaxFilterPBAsScoresNames && Idx < filtsrcpbascores->count; Idx++)
				{
				pszIncludeSrcPBAsScores[Idx] = nullptr;
				if (pszIncludeSrcPBAsScores[NumIncludeSrcPBAsScores] == nullptr)
					pszIncludeSrcPBAsScores[NumIncludeSrcPBAsScores] = new char[cMaxLenRE + 1];
				strncpy(pszIncludeSrcPBAsScores[NumIncludeSrcPBAsScores], filtsrcpbascores->sval[Idx], cMaxLenRE + 1);
				pszIncludeSrcPBAsScores[NumIncludeSrcPBAsScores][cMaxLenRE + 1] = '\0';
				CUtility::TrimQuotedWhitespcExtd(pszIncludeSrcPBAsScores[NumIncludeSrcPBAsScores]);
				if (pszIncludeSrcPBAsScores[NumIncludeSrcPBAsScores][0] != '\0')
					NumIncludeSrcPBAsScores++;
				}
			}
		else
			NumIncludeSrcPBAsScores = 0;
...
// show user current resource limits
	...
	if(PMode == eMCSHGroupScores)
		{
		MinUnprunedRefs = minunprunedrefs->count ? minunprunedrefs->ival[0] : cDfltMinUnprunedRefs;
		if(MinUnprunedRefs < 1)
			MinUnprunedRefs = 1;
		else
			{
			MaxUnprunedRefs = maxunprunedrefs->count ? maxunprunedrefs->ival[0] : cDfltMaxUnprunedRefs;
			if(MaxUnprunedRefs < MinUnprunedRefs)
				MaxUnprunedRefs = MinUnprunedRefs;
			}
		}
	...
	switch (PMode) {
		...
		case eMCSHKFiltScores:
			pszDescr = "Filter scores file by source and reference PBAs names into new filtered scores file";
			break;
		case eMCSHKTransFiltScores:
			pszDescr = "Filter scores file by source and reference PBAs names into new transformed filtered scores file";
			break;
		}
	...
	if(PMode != eMCSHKMerGrps && PMode < eMCSHKFiltScores)
		{
	...
	if(PMode == eMCSHGroupScores || PMode >= eMCSHKFiltScores)
		{
		if(MinUnprunedRefs > 0)
			{
			gDiagnostics.DiagOutMsgOnly(eDLInfo, "Prune whilst at least this number of unpruned references remain: %d", MinUnprunedRefs);
			gDiagnostics.DiagOutMsgOnly(eDLInfo, "Attempt to prune untill no more than this number of unpruned references : %d", MaxUnprunedRefs);
			}
		gDiagnostics.DiagOutMsgOnly(eDLInfo, "Loading association scores for grouping from: '%s'", szAlleleScoreFile);
		}
	else
		{
		if (PMode == eMCSHSrcVsRefs || PMode == eMCSHRefsVsRefs)
	...
	if(PMode >= eMCSHKFiltScores)
		{
		if (NumIncludeSrcPBAsScores > 0)
			for (Idx = 0; Idx < NumIncludeSrcPBAsScores; Idx++)
				gDiagnostics.DiagOutMsgOnly(eDLInfo, "Retaining source PBA names : '%s'", pszIncludeSrcPBAsScores[Idx]);
		else
			gDiagnostics.DiagOutMsgOnly(eDLInfo, "Retaining source PBA names : 'All'");

		if(NumIncludeRefPBAsScores > 0)
			for (Idx = 0; Idx < NumIncludeRefPBAsScores; Idx++)
				gDiagnostics.DiagOutMsgOnly(eDLInfo, "Retaining reference PBA names : '%s'", pszIncludeRefPBAsScores[Idx]);
		else
			gDiagnostics.DiagOutMsgOnly(eDLInfo, "Retaining reference PBA names : 'All'");
		}
	...
	Rslt = CallHaplotypes(PMode,			// processing mode 0: report imputation haplotype matrix, 1: report both raw and imputation haplotype matrices, 2: additionally generate GWAS allowing visual comparisons, 3: allelic haplotype grouping,4: coverage haplotype grouping, 5: post-process haplotype groupings for DGTs,  6: post-process to WIG,  7 progeny PBAs vs. founder PBAs allelic association scores, 8 founder PBAs vs. founder PBAs allelic association scores, 9 grouping by scores, 10 differential group KMers
					...
					MinUnprunedRefs,		// in mode eMCSHGroupScores: prune whilst at least this number of unpruned references remain
					MaxUnprunedRefs,		// in mode eMCSHGroupScores: attempt to prune untill no more than this number of unpruned references
					...
					NumIncludeSrcPBAsScores,	// if filtering scores then only retain scores for this number of source PBAs
					pszIncludeSrcPBAsScores,	// names of source PBAs (could be wildcarded)

					NumIncludeRefPBAsScores,	// if filtering scores then only retain scores for this number of reference PBAs
					pszIncludeRefPBAsScores,	// names of reference PBAs (could be wildcarded)
...
Rslt = pCallHaplotypes->Process(PMode,ExprID,SeedRowID,LimitPrimaryPBAs, AffineGapLen,GrpHapBinSize,NumHapGrpPhases,MinCentClustDist,MaxCentClustDist,MaxClustGrps, SparseRepPropGrpMbrs, SparseRepProp,
				...
				FndrTrim5, FndrTrim3, ProgTrim5, ProgTrim3, WWRLProxWindow,OutliersProxWindow,MinUnprunedRefs,MaxUnprunedRefs,
				pszMaskBPAFile,pszChromFile,pszAlleleScoreFile, NumIncludeSrcPBAsScores, pszIncludeSrcPBAsScores, NumIncludeRefPBAsScores, pszIncludeRefPBAsScores,NumFounderInputFiles,
...
if (m_pBinScores != nullptr)
	{
#ifdef _WIN32
	free(m_pBinScores);				// was allocated with malloc/realloc, or mmap/mremap, not c++'s new....
#else
	if (m_pBinScores != MAP_FAILED)
		munmap(m_pBinScores, m_AllocdBinScoresMem);
#endif
	}
...
m_UsedNumBinScores = 0;				// current number of bin scores used 
m_AllocdBinScores = 0;				// current allocation is for this many 
m_AllocdBinScoresMem = 0;			// current mem allocation size for m_pBinScores
...
if(PMode >= eMCSHKFiltScores)
	{
	// compile source/reference PBA names regexprs
	if (Rslt = (m_RegExprs.CompilePBANamesREs(NumIncludeSrcPBAsScores, pszIncludeSrcPBAsScores, NumIncludeRefPBAsScores, pszIncludeRefPBAsScores)) < eBSFSuccess)
		{
		Reset();
		return(Rslt);
		}
	...
	if(PMode == eMCSHKFiltScores)
		Rslt = FilterAlleleScores(pszAlleleScoreFile,pszOutFile);
	else
		Rslt = FilterTransformAlleleScores(pszAlleleScoreFile, pszOutFile);
...
if (PMode == eMCSHGroupScores)
{
	Rslt = GroupAlleleScores(MinUnprunedRefs,MaxUnprunedRefs,pszAlleleScoreFile, pszMaskBPAFile,pszOutFile);
	Reset();
	return(Rslt);
}
...
if(!(m_PMode == eMCSHAllelicHapsGrps || m_PMode == eMCSHCoverageHapsGrps || m_PMode == eMCSHDGTHapsGrps))
	...
	if(m_pszMaskBPAFile != nullptr)
		...
		for(int FileID = 0; Rslt >= eBSFSuccess && FileID < NumFiles; ++FileID)
			...
			for(HGAllocID = 1; HGAllocID <= m_UsedHGBinSpecs; HGAllocID++, pHGBinSpec++)
				{
				pHGBinSpec->AllocID = HGAllocID;
				if(pHGBinSpec->ChromID != CurChromID)
					{
					CurChromID = pHGBinSpec->ChromID;
					pChromMetadata = LocateChromMetadataFor(1, pHGBinSpec->ChromID);
					pChromMetadata->HGBinID = HGAllocID; // first bin on this chromosome 
					}
				}
			}
		}
...
for (ChromIdx = 0; ChromIdx < pFndrMetadata->NumChroms && CurChromMetadataIdx != 0; ChromIdx++)
	...
	for (PBAsID = 1; PBAsID <= (NumRefPBAs + NumSrcPBAs); PBAsID++)
		...
		if ((pChromMetadata = LocateChromMetadataFor(PBAsID, CurChromID)) == nullptr)
			...
			{
			if((pFndrPBAs[PBAsID - 1] = pChromMetadata->pPBAs)==nullptr) // could be assigning nullptr if no chromosome reads but subsequent processing will check and handle appropriately
...
pHGBinSpec->AllocID = m_UsedHGBinSpecs;
ReleaseFastSerialise();
...
CCallHaplotypes::IterateHGBinSpecs(int32_t PrevAllocID,    // previously iterated bin identifier, to start from 1st bin then pass 0 as the bin identifier 
								   int32_t ChromID) // only processing bins on this chrom, 0 if processing all bins 
{
tsHGBinSpec* pHGBinSpec;

AcquireFastSerialise();
while(PrevAllocID < m_UsedHGBinSpecs)
	{
	pHGBinSpec = &m_pHGBinSpecs[PrevAllocID++]; // note: bin specs are unordered so need to iterate over all bins so as to ensure all bins on requested chrom are processed 
	...
	if(!(pHGBinSpec->ProcState & 0x07)) // bin processing state  - 0x00 if unprocessed, 0x01 if currently being processing, 0x03 if processing completed but referenced chromosome not located, 0x07 if processing successfully completed
		...
		m_ProccessingHGAllocID = pHGBinSpec->AllocID; // marks last bin accepted for processing, will be used to initialise processing for next chromosome
		ReleaseFastSerialise();
		return(pHGBinSpec);
		}
	}
...
gDiagnostics.DiagOut(eDLInfo, gszProcName, "LoadRefGroupings: Loading reference haplotype grouping bins for processing from file '%s'", pszInFile);
...
gDiagnostics.DiagOut(eDLInfo, gszProcName, "LoadRefGroupings: Parsed and accepted '%d' reference haplotype grouping bins for processing from file '%s'", NumAcceptedBins, pszInFile);
...
CurAllocID = m_ProccessingHGAllocID;
Rslt = eBSFSuccess;
while(Rslt >= eBSFSuccess) {
	...
	if(m_PMode < eMCSHAllelicHapsGrps) 
		...
		if((pCurBinSpec = IterateHGBinSpecs(CurAllocID,pWorkQueueEl->ChromID)) == nullptr)
			break;
		CurAllocID = pCurBinSpec->AllocID;
...
for(ChromIdx = 0; ChromIdx < pReadsetMetadata->NumChroms && CurChromMetadataIdx != 0; ChromIdx++)
	...
	while (WorkerThreadStatus > 0)
		...
		AcquireFastSerialise();
		int32_t LatestAllocID = m_ProccessingHGAllocID;
		ReleaseFastSerialise();
		gDiagnostics.DiagOut(eDLInfo, gszProcName, "AlignFounderHaps: Generating haplotype groupings on chromosome '%s' ... %0.3f%% of total haplotype grouping specification bins processed", pszChrom, (LatestAllocID * 100.0) / m_UsedHGBinSpecs);
		}
...
pASBin->AllocID = m_UsedASBins;
ReleaseFastSerialise();
return(m_UsedASBins);
}

tsASBin*    // returned allele scoring allocated bin specification, returns nullptr if all bins have been iterated
CCallHaplotypes::IterateASAllocIDs(int32_t PrevAllocID,    // previously iterated bin identifier, to start from 1st bin then pass 0 as the bin identifier 
	int32_t ChromID) // only processing bins on this chrom, 0 if processing all bins 
{
tsASBin* pASBin;

AcquireFastSerialise();
while (PrevAllocID < m_UsedASBins)
	{
	pASBin = &m_pASBins[PrevAllocID++]; // note: bin specs are unordered so need to iterate over all bins so as to ensure all bins on requested chrom are processed 
	...
	if (!(pASBin->ProcState & 0x07)) // bin processing state  - 0x00 if unprocessed, 0x01 if currently being processing, 0x03 if processing completed but referenced chromosome not located, 0x07 if processing successfully completed
		...
		m_ProccessingASAllocID = pASBin->AllocID; // marks last bin accepted for processing, will be used to initialise processing for next chromosome
		ReleaseFastSerialise();
		return(pASBin);
		}
	}
...
int
CCallHaplotypes::GroupAlleleScores(int32_t MinUnprunedRefs,		// in mode eMCSHGroupScores: prune whilst at least this number of unpruned references remain
						int32_t MaxUnprunedRefs,		// in mode eMCSHGroupScores: attempt to prune untill no more than this number of unpruned references
...
TotNumChromBins = 0;
	...
	// Note: there is an assumption in this processing that the scores file has been sorted by Chrom.SourcePBA.ReferencePBA.Bin ascending. This is the ordering used when the scores file was originally generated
while ((Rslt = m_pAlleleScoresCSVFile->NextLine()) > 0)		// onto next line containing fields
	...
	Rslt = m_pAlleleScoresCSVFile->GetInt(4, &BinID);
	if (Rslt < 0 || BinStartLoci < 0)
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "GroupAlleleScores: Error whilst parsing bin identifier %d at line %zd in file '%s'", BinID, CurLineNumber, pszInFile);
		Reset();
		return(eBSFerrParse);
		}
	...
	Rslt = m_pAlleleScoresCSVFile->GetInt(7, &AlignLen);
	if (Rslt < 0 || AlignLen < 0)
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "GroupAlleleScores: Error whilst parsing alignment length %d at line %zd in file '%s'", AlignLen, CurLineNumber, pszInFile);
		Reset();
		return(eBSFerrParse);
		}
	...
	if (AlignLen > BinSize)
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "GroupAlleleScores: Alignment length > Bin size at line %zd in file '%s'", AlignLen, BinSize, CurLineNumber, pszInFile);
		Reset();
		return(eBSFerrParse);
		}
	...
	if((ASBinIdx = AddASBin(SrcID,RefID,CurChromID, m_ChromSizes[CurChromID - 1], BinID, BinStartLoci, BinSize, AlignLen, ExactScore,PartialScore)) < 1)
		{
...
m_pAlleleScoresCSVFile = nullptr;
	...
	// sorting by ChromID.BinID.SrcID.RefID ascending
if (m_UsedASBins > 1)
	{
	m_mtqsort.SetMaxThreads(m_NumThreads);
	m_mtqsort.qsort(m_pASBins, m_UsedASBins, sizeof(tsASBin), SortASBins);
	}
else
	if(m_UsedASBins < 1)
		{
...
// Must be same number of bins at each loci as there are source * referenced scored samples
// Bins must be non-overlaping with no gaps (be contiguous) along full length of each chromosome
...
for (BinIdx = 0; BinIdx < m_UsedASBins; BinIdx++, pASBin++)
	{
	if(pASBin->ChromID != CurChromID)	// new chromosome starting?
		{
		if(CurChromID != 0)				// not the first chromosome?
			{
		...
		// chromosome (could be 1st) is starting
		CurChromID = pASBin->ChromID;
		if((CurBinLoci = pASBin->BinLoci) != 0)		// bins must cover full length of chromosome so 1st bin must start at loci 0
			break;
		if((CurBinID = pASBin->BinID) != 1)			// 1st bin expected to have identifier 1
			break;
	...
	if(pASBin->BinLoci != CurBinLoci)						// starting a new bin loci on same chrom
		{
		...
		if(ExpNxtBinLoci != pASBin->BinLoci)				// loci must be same as expected
			break;
		if (++CurBinID != pASBin->BinID)					// next bin so check bin identifier
			break;
	...
	NumBinsAtLoci++;									// same loci, additional bin
	if (CurBinID != pASBin->BinID)						// same bin loci so expecting same bin identifier
		break;
	if (CurChromSize != pASBin->ChromSize)				// bin on same chromosome so expecting to be same size
		break;
	if (NumBinsAtLoci > ExpBinsAtLoci)					// ensuring no more bins at each loci than expected
		break;
	if(CurBinSize != pASBin->BinSize)					// and bins are same size
		break;
	}

if (BinIdx < m_UsedASBins || NumBinsAtLoci != ExpBinsAtLoci || ExpNxtBinLoci != CurChromSize)
	{
	gDiagnostics.DiagOut(eDLFatal, gszProcName, "GroupAlleleScores: Inconsistencies in file '%s'", pszInFile);
	return(eBSFerrParse);
	}
...
if((Rslt = LoadRefGroupings(pszWGSHapGrpFile)) <= eBSFSuccess)
	{
	if(Rslt == 0)
		gDiagnostics.DiagOut(eDLWarn, gszProcName, "GroupAlleleScores: Nothing to do, no accepted WGS reference groupings accepted from file '%s'", pszWGSHapGrpFile);
	else
...
Rslt = ProcessAlleleScoreBins(pszOutFile,MinUnprunedRefs,MaxUnprunedRefs);

return(Rslt);
}

// FilterAlleleScores
// Parses an existing allele association scores file and generates a new scores file only containing scores for a
// specified set of source and reference PBAs
int
CCallHaplotypes::FilterAlleleScores(char* pszInFile, // existing CSV file containing allele association scores
...
bool bAcceptSrcRefBins;
char* pszCurSrcPBA;
char* pszCurRefPBA;
char* pszChrom;

int32_t CurScoreRowLen;
...
if ((m_pAlleleScoresCSVFile = new CCSVFile) == nullptr)
	{
	gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterAlleleScores: Unable to instantiate CCSVfile");
	Reset();
	return(eBSFerrObj);
	}
m_pAlleleScoresCSVFile->SetMaxFields(14); // only processing 1st 13 fields, allowing 1 extra so can detect if more thasn 13
...
if ((Rslt = m_pAlleleScoresCSVFile->Open(pszInFile)) != eBSFSuccess)
	{
	gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterAlleleScores: Unable to open file: '%s'", pszInFile);
	Reset();
	return(Rslt);
	}
...
if (ftruncate(m_hOutFile, 0) != 0)
	{	
	gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterAlleleScores: Unable to create/truncate %s - %s", pszOutFile, strerror(errno));
	Reset();
	return(eBSFerrCreateFile);
	}
#endif
...
if (m_pszOutBuffer == nullptr)
	...
	if ((m_pszOutBuffer = new uint8_t[cOutBuffSize]) == nullptr)
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterAlleleScores: Memory allocation of %u bytes for filtered scores output buffering failed - %s", cOutBuffSize, strerror(errno));
		Reset();
		return(eBSFerrMem);
		};
	m_AllocOutBuff = cOutBuffSize;
...
CurBin = 0;
FirstBin = 0;
bAcceptSrcRefBins = false;
pszCurSrcPBA = nullptr;
pszCurRefPBA = nullptr;

// 
	// allele association scores file structure
	// "SourcePBA","ReferencePBA","Chrom","Bin","BinLoci","BinSize","AlignLen","NumExactMatches","NumBiallelicExactMatches","NumPartialMatches","NumNonRefAlleles","ExactScore","PartialScore"
	// Note: there is an assumption in this processing that the scores file has been sorted by Chrom.SourcePBA.ReferencePBA.Bin ascending. This is the ordering used when the scores file was originally generated.
gDiagnostics.DiagOut(eDLInfo, gszProcName, "FilterAlleleScores: Opened and parsing scores file - '%s'", pszInFile);
time_t Started = time(0);
while ((Rslt = m_pAlleleScoresCSVFile->NextLine()) > 0)		// onto next line containing fields
	{
	CurLineNumber++;

	// is it time to let user know progress, only check every 100 lines
	if ((CurLineNumber % 100) == 0)
		...
		if (ElapsedSecs >= 60)
			{
			gDiagnostics.DiagOut(eDLInfo, gszProcName, "Progress - lines parsed %zd", CurLineNumber);
			Started = Now;
			}
		}

	if ((NumFields = m_pAlleleScoresCSVFile->GetCurFields()) < 13)	// must contain at least 13 fields, could be more but only interested in 1st 13
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterAlleleScores: Allele association score file '%s' expected to contain a minimum of 13 fields, it contains %d at line %zd", pszInFile, NumFields, CurLineNumber);
		Reset();
		return(eBSFerrParse);
		}

		// 1st row may be a header row, if so then skip this row. If not transforming then save for writing out to the filtered output CSV file
	if (CurLineNumber == 1 && m_pAlleleScoresCSVFile->IsLikelyHeaderLine())
		{
		CurScoreRowLen = m_pAlleleScoresCSVFile->GetLine(m_AllocOutBuff - m_OutBuffIdx, (char *) &m_pszOutBuffer[m_OutBuffIdx]);
		m_OutBuffIdx += CurScoreRowLen;
		continue;
		}

	Rslt = m_pAlleleScoresCSVFile->GetInt(4, &CurBin);
	// if CurBin <= PrevBin then, because of scores ordering assumption, then could be starting a new chromosome, or a new SourcePBA, or a new ReferencePBA
	if(FirstBin == 0 || CurBin <= FirstBin)
		{
		Rslt = m_pAlleleScoresCSVFile->GetText(3, &pszChrom);
		if (Rslt < 0 || pszChrom == nullptr || pszChrom[0] == '\0')
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterAlleleScores: Unable to parse chromosome name at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}
		if (!AcceptThisChromName(pszChrom)) // sloughing until non-filtered chromosome
			{
			FirstBin = 0;
			CurBin = 0;
			bAcceptSrcRefBins = false;
		...
		FirstBin = CurBin;
		bAcceptSrcRefBins = true;	// assumption is that will be accepting bins 1..N if first bin has accepted combination of source and reference PBAs
		}

	if(!bAcceptSrcRefBins)
		continue;

	// accepting bins on pszChrom
	// if would be first bin on chromosome then check if accepting the combination of source and reference PBAs
	if(CurBin == FirstBin)
		{
		bAcceptSrcRefBins = false;	// set to true if discovered that will be accepting combination of source and reference for bins 1..N
		Rslt = m_pAlleleScoresCSVFile->GetText(1, &pszCurSrcPBA);
		if (Rslt < 0 || pszCurSrcPBA == nullptr || pszCurSrcPBA[0] == '\0')
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterAlleleScores: Unable to parse source identifier at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}
		if (!m_RegExprs.MatchSrcPBANameRxprs(pszCurSrcPBA))
			continue;

		Rslt = m_pAlleleScoresCSVFile->GetText(2, &pszCurRefPBA);
		if (Rslt < 0 || pszCurRefPBA == nullptr || pszCurRefPBA[0] == '\0')
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterAlleleScores: Unable to parse reference identifier at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}
		if (!m_RegExprs.MatchRefPBANameRxprs(pszCurRefPBA))
			continue;
		bAcceptSrcRefBins = true;
		}

	if(CurLineNumber > 1)
		m_pszOutBuffer[m_OutBuffIdx++] = '\n';
	CurScoreRowLen = m_pAlleleScoresCSVFile->GetLine(m_AllocOutBuff - m_OutBuffIdx, (char*)&m_pszOutBuffer[m_OutBuffIdx]);
	m_OutBuffIdx += CurScoreRowLen;
	if((m_OutBuffIdx + 10000) >= m_AllocOutBuff)
		{
		if (!CUtility::RetryWrites(m_hOutFile, m_pszOutBuffer, m_OutBuffIdx))
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterAlleleScores: Fatal error in RetryWrites()");
			Reset();
			return(eBSFerrFileAccess);
			}
		m_OutBuffIdx = 0;
...
gDiagnostics.DiagOut(eDLInfo, gszProcName, "FilterAlleleScores: Completed parsing and filtering %zd lines from input scores file: '%s'", CurLineNumber,pszInFile);
delete m_pAlleleScoresCSVFile;
m_pAlleleScoresCSVFile = nullptr;
if(m_hOutFile != -1)
	{
...
gDiagnostics.DiagOut(eDLInfo, gszProcName, "FilterAlleleScores: Completed writing filtered scores to file: '%s'", pszOutFile);
return(Rslt);
}


// FilterTransformAlleleScores
// Parses an existing allele association scores file and generates a new scores file with transformation
// Each row will contain values for all retained references
int
CCallHaplotypes::FilterTransformAlleleScores(char* pszInFile, // existing CSV file containing allele association scores
											char* pszOutFile)								 // where to write source and/or reference named filtered allele association scores CSV file
{
	int32_t Rslt;
	int64_t CurLineNumber;
	...
	int32_t NumRefIDs;
	int32_t BinIdx;
	tsCHChromScores *pScores;
	tsCHChromScores *pAnchorScores;

	int32_t CurScoreRowLen;

	size_t memreq;

	if (m_pBinScores == nullptr)			// initial allocation required?
		{
		// initial allocation, will be realloc'd as required if more memory required
		memreq = (size_t)cAllocHGBinSpecs * sizeof(tsCHChromScores);
#ifdef _WIN32
		m_pBinScores = (tsCHChromScores*)malloc(memreq);	// initial and perhaps the only allocation
		if (m_pBinScores == nullptr)
		{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "Initial memory allocation of %zd bytes for chromosome metadata failed - %s", (int64_t)memreq, strerror(errno));
			Reset();
...
#else
		m_pBinScores = (tsCHChromScores*)mmap(nullptr, memreq, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
		if (m_pBinScores == MAP_FAILED)
		{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "Process: Memory allocation of %zd bytes through mmap() for chromosome metadata failed - %s", (int64_t)memreq, strerror(errno));
			m_pBinScores = nullptr;
...
#endif
		m_AllocdBinScoresMem = memreq;
		m_AllocdBinScores = cAllocHGBinSpecs;
		}
	...
	if ((m_pAlleleScoresCSVFile = new CCSVFile) == nullptr)
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to instantiate CCSVfile");
		Reset();
		return(eBSFerrObj);
		}
	m_pAlleleScoresCSVFile->SetMaxFields(14); // only processing 1st 13 fields, allowing 1 extra so can detect if more thasn 13
	...
	if ((Rslt = m_pAlleleScoresCSVFile->Open(pszInFile)) != eBSFSuccess)
	{
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to open file: '%s'", pszInFile);
		Reset();
		return(Rslt);
	}
	...
	if ((m_hOutFile = open64(pszOutFile, O_WRONLY | O_CREAT, S_IREAD | S_IWRITE)) != -1)
		if (ftruncate(m_hOutFile, 0) != 0)
		{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to create/truncate %s - %s", pszOutFile, strerror(errno));
			Reset();
			return(eBSFerrCreateFile);
		}
#endif
	...
	if (m_pszOutBuffer == nullptr)
	{
		if ((m_pszOutBuffer = new uint8_t[cOutBuffSize]) == nullptr)
		{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Memory allocation of %u bytes for filtered scores output buffering failed - %s", cOutBuffSize, strerror(errno));
			Reset();
			return(eBSFerrMem);
		};
		m_AllocOutBuff = cOutBuffSize;
	...
	CurLineNumber = 0;
	CurScoreRowLen = 0;
	m_UsedNumBinScores = 0;
	pScores = m_pBinScores;
	time_t Started = time(0);
	gDiagnostics.DiagOut(eDLInfo, gszProcName, "FilterTransformAlleleScores: Opened and parsing scores file - '%s'", pszInFile);
	...
	while ((Rslt = m_pAlleleScoresCSVFile->NextLine()) > 0)		// onto next line containing fields
		...
		if((CurLineNumber % 100) == 0)
			...
			if (ElapsedSecs >= 60)
				{
				gDiagnostics.DiagOut(eDLInfo, gszProcName, "Progress - parsed %zd lines", CurLineNumber);
				Started = Now;
				}
			}
		...
		if ((NumFields = m_pAlleleScoresCSVFile->GetCurFields()) < 13)	// must contain at least 13 fields, could be more but only interested in 1st 13
		{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Allele association score file '%s' expected to contain a minimum of 13 fields, it contains %d at line %zd", pszInFile, NumFields, CurLineNumber);
			Reset();
			return(eBSFerrParse);
		}
		...
		if (FirstBin == 0 || CurBin <= FirstBin)
			...
			if (Rslt < 0 || pszChrom == nullptr || pszChrom[0] == '\0')
				{
				gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse chromosome name at line %zd in file '%s'", CurLineNumber, pszInFile);
				Reset();
				return(eBSFerrParse);
				}
			if (!AcceptThisChromName(pszChrom)) // sloughing until non-filtered chromosome
		...
		if (CurBin == FirstBin)
			...
			if (Rslt < 0 || pszSrcPBA == nullptr || pszSrcPBA[0] == '\0')
				{
				gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse source identifier at line %zd in file '%s'", CurLineNumber, pszInFile);
				Reset();
				return(eBSFerrParse);
				}

			if (!m_RegExprs.MatchSrcPBANameRxprs(pszSrcPBA))
				continue;
			Rslt = m_pAlleleScoresCSVFile->GetText(2, &pszRefPBA);
			if (Rslt < 0 || pszRefPBA == nullptr || pszRefPBA[0] == '\0')
				{
				gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse reference identifier at line %zd in file '%s'", CurLineNumber, pszInFile);
				Reset();
				return(eBSFerrParse);
				}
			if (!m_RegExprs.MatchRefPBANameRxprs(pszRefPBA))
				continue;
			bAcceptSrcRefBins = true;
			}

		// needing to realloc to hold more bin scores?
		if(m_UsedNumBinScores == m_AllocdBinScores)
			{
			int32_t ToAllocdBinScores = m_AllocdBinScores + cReallocHGBinSpecs;
			size_t memreq = (size_t)ToAllocdBinScores * sizeof(tsCHChromScores);
#ifdef _WIN32
			pScores = (tsCHChromScores*)realloc(m_pBinScores, memreq);
			if (pScores == nullptr)
			{
#else
			pScores = (tsCHChromScores*)mremap(m_pBinScores, m_AllocdBinScoresMem, memreq, MREMAP_MAYMOVE);
			if (pScores == MAP_FAILED)
			{
#endif
				gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Memory reallocation to %zd bytes failed - %s", (int64_t)memreq, strerror(errno));
				return(eBSFerrMem);
			}
			m_pBinScores = pScores;
			m_AllocdBinScoresMem = memreq;
			m_AllocdBinScores = ToAllocdBinScores;
			pScores = &m_pBinScores[m_UsedNumBinScores];
			}

		if((pScores->ChromID = AddChrom(pszChrom)) == 0)
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to accept parsed chrom '%s' at line %zd in file '%s'", pszChrom, CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}

		if((pScores->RefID = AddReadset(pszRefPBA, 0))==0)
			pScores->RefID = LocateReadset(pszRefPBA, 0);
		if((pScores->SrcID = AddReadset(pszSrcPBA, 1))==0)
			pScores->SrcID = LocateReadset(pszSrcPBA, 1);

		Rslt=  m_pAlleleScoresCSVFile->GetInt(4, &pScores->BinID);
		if (Rslt < 0)
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse Bin identifier at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}

		Rslt = m_pAlleleScoresCSVFile->GetInt(5, &pScores->BinLoci);
		if (Rslt < 0)
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse Bin Loci at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}
		Rslt = m_pAlleleScoresCSVFile->GetInt(6, &pScores->BinSize);
		if (Rslt < 0)
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse Bin Size at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}

		Rslt = m_pAlleleScoresCSVFile->GetInt(7, &pScores->AlignLen);
		if (Rslt < 0)
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse AlignLen at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}

		Rslt = m_pAlleleScoresCSVFile->GetInt(8, &pScores->NumExactMatches);
		if (Rslt < 0)
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse NumExactMatches at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}

		Rslt = m_pAlleleScoresCSVFile->GetInt(9, &pScores->NumBiallelicExactMatches);
		if (Rslt < 0)
		{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse NumBiallelicExactMatches at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
		}
		Rslt = m_pAlleleScoresCSVFile->GetInt(10, &pScores->NumPartialMatches);
		if (Rslt < 0)
		{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse NumPartialMatches at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
		}
		Rslt = m_pAlleleScoresCSVFile->GetInt(11, &pScores->NumNonRefAlleles);
		if (Rslt < 0)
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse NumNonRefAlleles at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}

		Rslt = m_pAlleleScoresCSVFile->GetDouble(12, &pScores->ExactScore);
		if (Rslt < 0)
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse ExactScore at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}
		Rslt = m_pAlleleScoresCSVFile->GetDouble(13, &pScores->PartialScore);
		if (Rslt < 0)
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Unable to parse PartialScore at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}
		m_UsedNumBinScores += 1;
		pScores++;
		if(m_UsedNumBinScores > 1000000000)		// need to have a limit!
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Increase filtering stringency as exceeding limit of 1 billion accepted parsed bins at line %zd in file '%s'", CurLineNumber, pszInFile);
			Reset();
			return(eBSFerrParse);
			}
	}
	...
	if(m_UsedNumBinScores == 0)
		{
		gDiagnostics.DiagOut(eDLWarn, gszProcName, "FilterTransformAlleleScores: No accepted scores after parsing and filtering %zd lines in file '%s'", CurLineNumber, pszInFile);
		// commit output file
#ifdef _WIN32
		_commit(m_hOutFile);
#else
	...
	gDiagnostics.DiagOut(eDLInfo, gszProcName, "FilterTransformAlleleScores: Parsing completed, accepted %d bins parsed from %zd lines in file '%s'", m_UsedNumBinScores, CurLineNumber, pszInFile);
	gDiagnostics.DiagOut(eDLInfo, gszProcName, "FilterTransformAlleleScores: Sorting accepted bins");

	// sort bins 
	// sorting by ChromID.BinID.SrcID.RefID ascending
	if (m_UsedNumBinScores > 1)
		m_mtqsort.qsort(m_pBinScores, m_UsedNumBinScores, sizeof(tsCHChromScores), SortChromScores);

	gDiagnostics.DiagOut(eDLInfo, gszProcName, "FilterTransformAlleleScores: Sorting completed, now writing to file: %s",pszOutFile);

	m_OutBuffIdx = sprintf((char*)m_pszOutBuffer, "\"SourcePBA\",\"Value\",\"Chrom\",\"Bin\",\"BinLoci\",\"BinSize\"");

	// iterate over initial bin for initial chromosome for initial source PBA and discover the number and ordering of reference PBAs
	// enabling generation of the filtered output header line
	pScores = m_pBinScores;
	pAnchorScores = pScores;
	NumRefIDs = 0;
	for(BinIdx = 0; BinIdx < m_UsedNumBinScores; BinIdx++, pScores++)
		{
		if(pScores->ChromID != pAnchorScores->ChromID)
			break;
		if (pScores->BinID != pAnchorScores->BinID)
			break;
		if (pScores->SrcID != pAnchorScores->SrcID)
			break;
		pszRefPBA = LocateReadset(pScores->RefID);
		NumRefIDs++;
		m_OutBuffIdx += sprintf((char *)&m_pszOutBuffer[m_OutBuffIdx],",\"%s\"", pszRefPBA);
		}
	...
	for(BinIdx = 0; BinIdx < m_UsedNumBinScores; BinIdx += NumRefIDs)
		{
		pAnchorScores = &m_pBinScores[BinIdx];
		pszSrcPBA = LocateReadset(pAnchorScores->SrcID);
		pszChrom = LocateChrom(pAnchorScores->ChromID);

		for (ValueType = 0; ValueType <= 6; ValueType++)
			{
			switch (ValueType) {
				case 0:
					pszValueType = (char *)"AlignLen";
					break;
				case 1:
					pszValueType = (char*)"NumExactMatches";
					break;
				case 2:
					pszValueType = (char*)"NumBiallelicExactMatches";
					break;
				case 3:
					pszValueType = (char*)"NumPartialMatches";
					break;
				case 4:
					pszValueType = (char*)"NumNonRefAlleles";
					break;
				case 5:
					pszValueType = (char*)"ExactScore";
					break;
				case 6:
					pszValueType = (char*)"PartialScore";
					break;
				}

			m_OutBuffIdx += sprintf((char *)&m_pszOutBuffer[m_OutBuffIdx],"\n\"%s\",\"%s\",\"%s\",%d,%d,%d", pszSrcPBA, pszValueType,pszChrom, pAnchorScores->BinID, pAnchorScores->BinLoci, pAnchorScores->BinSize);
			pScores = pAnchorScores;
			for (RefIdx = 0; RefIdx < NumRefIDs; RefIdx++,pScores++)
				{
				switch(ValueType) {
					case 0:
						IntValue = pScores->AlignLen;
						break;

					case 1:
						IntValue = pScores->NumExactMatches;
						break;

					case 2:
						IntValue = pScores->NumBiallelicExactMatches;
						break;

					case 3:
						IntValue = pScores->NumPartialMatches;
						break;

					case 4:
						IntValue = pScores->NumNonRefAlleles;
						break;

					case 5:
						DblValue = pScores->ExactScore;
						break;

					case 6:
						DblValue = pScores->PartialScore;
						break;
					}

					if(ValueType <= 4)	// integer types
						m_OutBuffIdx += sprintf((char*)&m_pszOutBuffer[m_OutBuffIdx], ",\"%d\"", IntValue);
					else
						m_OutBuffIdx += sprintf((char*)&m_pszOutBuffer[m_OutBuffIdx], ",\"%0.5f\"", DblValue);
					}
				}
			if ((m_OutBuffIdx + 10000) >= m_AllocOutBuff)
				{
				if (!CUtility::RetryWrites(m_hOutFile, m_pszOutBuffer, m_OutBuffIdx))
					{
					gDiagnostics.DiagOut(eDLFatal, gszProcName, "FilterTransformAlleleScores: Fatal error in RetryWrites()");
					Reset();
					return(eBSFerrFileAccess);
					}
				m_OutBuffIdx = 0;
	...
	gDiagnostics.DiagOut(eDLInfo, gszProcName, "FilterTransformAlleleScores: Completed writing to file: %s", pszOutFile);
	return(Rslt);
}



// Note: On entry the bins will have been sorted -
//		ChromID.BinLoci.SrcID.RefID ascending
int32_t
CCallHaplotypes::ProcessAlleleScoreBins(char* pszOutFile,				// where to write allele score bins grouping CSV file
	int32_t MinUnprunedRefs,	// prune whilst at least this number of unpruned references remain
	int32_t MaxUnprunedRefs)	// prune untill no more than this number of unpruned references
{
	int32_t Rslt;

	// ensure that noise pruning has reasonable min/max unpruned limits
	if(MinUnprunedRefs < 1)
		MinUnprunedRefs = 1;
	else
		if(MinUnprunedRefs > m_NumReadsetTypes[0])
			MinUnprunedRefs = m_NumReadsetTypes[0];


	if (MaxUnprunedRefs < MinUnprunedRefs)
		MaxUnprunedRefs = MinUnprunedRefs;
	else
		if (MaxUnprunedRefs > m_NumReadsetTypes[0])
			MaxUnprunedRefs = m_NumReadsetTypes[0];
	...
	int32_t NumRefIDs = m_NumReadsetTypes[0];
	int32_t NumSrcIDs = m_NumReadsetTypes[1];
	...
	int32_t CurRefID;
	int32_t CurSrcID;
	double CurScorePartial;

	double CurScoreExact;
	char* pszCurChrom;
	int64_t NumBinScoresAccepted;
	int64_t NumBinScoresImputed;
	int64_t NumBinScoresRejected;
	int64_t TotNumBinsImputeChkd;
	int64_t CurSummaryLine;
	double PropAligned;

	pASBin = m_pASBins;
	for (BinIdx = 0; BinIdx < m_UsedASBins; BinIdx++, pASBin++)		// initially starting as all bins to be processed for all reference scores
		pASBin->ProcState = 0x00;									// these are later set to 0x01 if bin score accepted without need for imputation
																	// or 0x02 if score was imputated
																	// or 0x03 if rejected, score could have been imputated but that imputation would have been from a bin also imputated 

// imputations, at chromosome level, for combinations of reference and source
CUtility::AppendFileNameSuffix(szSrcRefs, pszOutFile, (char*)".imputation.csv", '.');
	...
	Rslt = eBSFSuccess; // a big assumption!!!
	gDiagnostics.DiagOut(eDLInfo, gszProcName, "ProcessAlleleScoredBins: created or truncated imputation output file: '%s'", szSrcRefs);

	m_OutBuffIdx = sprintf((char*)&m_pszOutBuffer[0], "\"SummaryLine\",\"Chrom\",\"Source (GBS)\",\"Reference (WGS)\",\"PropAccepted\",\"PropImputed\",\"PropRejected\"");
	if (!CUtility::RetryWrites(m_hOutFile, m_pszOutBuffer, m_OutBuffIdx))
		{
	...
	// Note: bin ordering is ChromID.BinLoci.SrcID.RefID ascending
pszCurChrom = nullptr;
NumBinScoresAccepted = 0;
NumBinScoresImputed = 0;
NumBinScoresRejected = 0;
TotNumBinsImputeChkd = 0;
CurSummaryLine = 0;
pASBinMH = nullptr;
CurSrcID = 0;
CurRefID = 0;
PropAligned = 0.0;
for (SrcIdx = 0; SrcIdx < NumSrcIDs; SrcIdx++)						// iterating each source (usually the progeny GBS)
	{
	for (RefIdx = 0; RefIdx < NumRefIDs; RefIdx++)					// iterating each reference (usually the parental WGS) for the current source
		{
		CurChromID = 0;
		for (BinIdx = ((int64_t)SrcIdx * NumRefIDs) + RefIdx; BinIdx < m_UsedASBins; BinIdx += ((int64_t)NumRefIDs * NumSrcIDs))	// iterating each bin for the current source (SrcIdx) vs current reference (RefIdx)
			{
			pASBin = &m_pASBins[BinIdx];
			CurSrcID = pASBin->SrcID;
			CurRefID = pASBin->RefID;

			if (CurChromID > 0 && pASBin->ChromID != CurChromID)																	// writing to summary at end of each chromosome, same combination of (SrcIdx) vs current reference (RefIdx)
				{																													// when all chroms processed then processing starts for combination of (SrcIdx) vs next reference (RefIdx+1)
				// Log the proportions of imputations required
				CurSummaryLine += 1;
				m_OutBuffIdx += sprintf((char*)&m_pszOutBuffer[m_OutBuffIdx], "\n%zd,\"%s\",\"%s\",\"%s\",%f,%f,%f", CurSummaryLine,LocateChrom(CurChromID), LocateReadset(CurSrcID), LocateReadset(CurRefID),(double)NumBinScoresAccepted / TotNumBinsImputeChkd, (double)NumBinScoresImputed / TotNumBinsImputeChkd, (double)NumBinScoresRejected / TotNumBinsImputeChkd);
				if (m_OutBuffIdx + 10000 > m_AllocOutBuff)
					{
					if (!CUtility::RetryWrites(m_hOutFile, m_pszOutBuffer, m_OutBuffIdx))
						{
			...
			// processing bins for all references along same chromosome for same source
			CurChromID = pASBin->ChromID;
			TotNumBinsImputeChkd++;

			// if no alignments, or very small sized bin, for current bin then attempt to impute from nearest bin for same ChromID.SrcID.RefID
			// if nearest is more than 2 bins away then don't bother imputing
			PropAligned = ((double)pASBin->AlignLen / pASBin->BinSize);
			if (pASBin->BinSize < 10000 || PropAligned < 0.01)
				{
				if(pASBinMH != nullptr && pASBinMH->ProcState == cBinScoreAccepted)	// can only impute from previous ChromID.SrcID.RefID bin if that previous bin score was not imputed
					{
					NumBinScoresImputed++;
					pASBin->ScoreExact = pASBinMH->ScoreExact;
					pASBin->ScorePartial = pASBinMH->ScorePartial;
					pASBin->ProcState = cBinScoreImputed;									// marking score as having been imputed
					}
				else
					{
					NumBinScoresRejected++;
					pASBin->ProcState = cBinScoreNotAccepted;								// can't impute as either 1st bin on chrom or previous bin on same chrom was imputed
					}
				}
			else // bin doesn't require imputation, but previous (immediately left) bin may need imputation
				{
				pASBin->ProcState = cBinScoreAccepted;								// directly accepting bin score without any imputation
				NumBinScoresAccepted++;
				if (pASBinMH != nullptr && pASBinMH->ProcState == cBinScoreNotAccepted)
					{
					NumBinScoresImputed++;
					NumBinScoresRejected--;
					pASBinMH->ScoreExact = pASBin->ScoreExact;
					pASBinMH->ScorePartial = pASBin->ScorePartial;
					pASBinMH->ProcState = cBinScoreImputed;							 // marking previous bin scores as having been imputed because pASBin was not imputed
					}
				}
			if(pASBin->ProcState == 0)
				pASBinMH = nullptr;

			pASBinMH = pASBin;
			if((NumBinScoresImputed + NumBinScoresRejected + NumBinScoresAccepted) != TotNumBinsImputeChkd)
				gDiagnostics.DiagOut(eDLWarn, gszProcName, "ProcessAlleleScoreBins: Imputation failed!!!!");
			}
		// summary for combination of (SrcIdx) vs last chrom (CurChromID) reference (RefIdx) still needs writing out ....
		CurSummaryLine += 1;
		m_OutBuffIdx += sprintf((char*)&m_pszOutBuffer[m_OutBuffIdx], "\n%zd,\"%s\",\"%s\",\"%s\",%f,%f,%f", CurSummaryLine,
					LocateChrom(CurChromID), LocateReadset(CurSrcID), LocateReadset(CurRefID), (double)NumBinScoresAccepted / TotNumBinsImputeChkd, (double)NumBinScoresImputed / TotNumBinsImputeChkd, (double)NumBinScoresRejected / TotNumBinsImputeChkd);
		if((m_OutBuffIdx + 10000) > m_AllocOutBuff)
			{
			if (!CUtility::RetryWrites(m_hOutFile, m_pszOutBuffer, m_OutBuffIdx))
				{
...
// imputation summary file, genome wide for each source without regard to reference
CUtility::AppendFileNameSuffix(szSrcRefs, pszOutFile, (char*)".imputation.summary.csv", '.');
...
Rslt = eBSFSuccess; // a big assumption!!!
gDiagnostics.DiagOut(eDLInfo, gszProcName, "ProcessAlleleScoredBins: created or truncated imputation genome summary output file: '%s'", szSrcRefs);

m_OutBuffIdx = sprintf((char*)&m_pszOutBuffer[0], "\"SummaryLine\",\"Source (GBS)\",\"PropAccepted\",\"PropImputed\",\"PropRejected\"");
if (!CUtility::RetryWrites(m_hOutFile, m_pszOutBuffer, m_OutBuffIdx))
	{
	gDiagnostics.DiagOut(eDLFatal, gszProcName, "ProcessAlleleScoreBins: Fatal error in RetryWrites()");
	Reset();
...
CurSummaryLine = 0;
CurSrcID = 0;
for (SrcIdx = 0; SrcIdx < NumSrcIDs; SrcIdx++)						// iterating each source
	{
	NumBinScoresAccepted = 0;
	NumBinScoresImputed = 0;
	NumBinScoresRejected = 0;
	TotNumBinsImputeChkd = 0;
	// iterating each bin for the current source over complete genome - not interested in individual chromosomes
	for (BinIdx = ((int64_t)SrcIdx * NumRefIDs); BinIdx < m_UsedASBins; BinIdx += ((int64_t)NumRefIDs * NumSrcIDs))
		{
		pASBin = &m_pASBins[BinIdx];
		CurSrcID = pASBin->SrcID;
		for (RefIdx = 0; RefIdx < NumRefIDs; RefIdx++, pASBin++)					// iterating each reference for the current source bin
			{
			if(pASBin->ProcState & cBinScoreAccepted)
				NumBinScoresAccepted++;
			if (pASBin->ProcState & cBinScoreImputed)
				NumBinScoresImputed++;
			if (pASBin->ProcState & cBinScoreNotAccepted)
				NumBinScoresRejected++;
			TotNumBinsImputeChkd++;
			}
		}
	...
	CurSummaryLine += 1;
	m_OutBuffIdx += sprintf((char*)&m_pszOutBuffer[m_OutBuffIdx], "\n%zd,\"%s\",%f,%f,%f", CurSummaryLine,
		LocateReadset(CurSrcID), (double)NumBinScoresAccepted / TotNumBinsImputeChkd, (double)NumBinScoresImputed / TotNumBinsImputeChkd, (double)NumBinScoresRejected / TotNumBinsImputeChkd);
	if ((m_OutBuffIdx + 10000) > m_AllocOutBuff)
		{
		if (!CUtility::RetryWrites(m_hOutFile, m_pszOutBuffer, m_OutBuffIdx))
			{
...
CUtility::AppendFileNameSuffix(szSrcRefs, pszOutFile, (char*)".csv", '.');
...
gDiagnostics.DiagOut(eDLInfo, gszProcName, "ProcessAlleleScoredBins: Created or truncated founder instances references output file: '%s'", szSrcRefs);
pASBin = m_pASBins;

// iterate over each source, then over each reference for that source
// references are gradually marked and pruned from future processing until a subset of references remain
// pruning is based on the concept of reducing the number of potential references so as to maximise the number of bins in which the subset of remaining unpruned references are the highest scoring
// Pruning is predicated on the assumption that if a reference is highest scoring simply because of noise artifacts within that bin then it is expected that over a large number of bins (genome wide)
// then the noise will be normalised out such that the true highest scoring target references will have more bins in which they are highest relative to those targets which were only highest because of noise
// Determination of references for pruning is only on bins which are either directly accepted scores (cBinScoreAccepted) or imputed (cBinScoreImputed). Bins which couldn't be imputed (cBinScoreNotAccepted) are not considered during pruning processing.
// 
// Note: On entry the bins will have been sorted -
//		ChromID.BinLoci.SrcID.RefID ascending
int8_t* pPruneRefs = new int8_t[NumRefIDs];						// prune these references from the classifications
int32_t* pGenomeRefCnts = new int32_t[NumRefIDs];
double CurRefScoreDiff = 0.0;

double HighScorePartial = 0.0;
int32_t HighRefIdx = 0;
int32_t NumUnprunedRefs;
int32_t CurHighRefCntThres;

bool bPruning;
bool bSourceNoDiffRefs;

for (SrcIdx = 0; SrcIdx < NumSrcIDs; SrcIdx++)							// iterating each source
	{
	memset(pPruneRefs, 0, sizeof(int8_t) * NumRefIDs);
	bSourceNoDiffRefs = false;
	CurHighRefCntThres = 1;
	do {
		memset(pGenomeRefCnts, 0, sizeof(int32_t) * NumRefIDs);
		for (BinIdx = (int64_t)SrcIdx * NumRefIDs; BinIdx < m_UsedASBins; BinIdx += ((int64_t)NumSrcIDs * NumRefIDs))	// iterating each bin for the current source to first reference bin
			{
			pASBin = &m_pASBins[BinIdx];
			HighRefIdx = -1;
			HighScorePartial = pASBin->ScorePartial;						// default is for 1st reference to have highest score
			for (RefIdx = 0; RefIdx < NumRefIDs; RefIdx++, pASBin++)		// iterating each reference for current source
				{
				if(bSourceNoDiffRefs)
					pASBin->ProcState |= cBinSourceNoDiffRefs;

				if (pASBin->ProcState & cBinRefPruned)							// already decided that this individual bin's reference is to be pruned
					continue;

				if(pPruneRefs[RefIdx] & 0x01)								// already decided to prune this reference from all bins?
					{
					pASBin->ProcState |= cBinRefPruned;						// mark individual bin as a pruned reference
					continue;
					}

				if (pASBin->ProcState & cBinScoreNotAccepted)				// these are bins which couldn't be imputed (cBinScoreImputed) from bracketing bins with scores directly accepted as not requiring imputation (cBinScoreAccepted)
					{
					pASBin->ProcState |= cBinRefPruned;						// mark individual bin as a pruned bin, but reference may still be in contention as was not marked as a pruned reference in pPruneRefs[]!
					continue;
					}

				// bin reference currently unpruned and it's score was either cBinScoreAccepted or cBinScoreImputed 
				if(pASBin->ScorePartial >= HighScorePartial)				// highest scoring bin (RefIdx) for current source (SrcIdx)?
					{
					HighScorePartial = pASBin->ScorePartial;
					HighRefIdx = RefIdx;
					}
				}
			if(HighRefIdx != -1)
				pGenomeRefCnts[HighRefIdx]++;
			}

		if(bSourceNoDiffRefs)		// if already known that all references were pruned because of no score differentials over complete genome then bail out ...
			break;

		// now know how many times each reference was the highest scoring for the current source over all bins in the genome for the current source (SrcIdx)
		// if a reference had very few high scoring bins then that reference is treated as pure noise and reference will be marked for pruning in next iteration
		bPruning = true;						// default is to assume that another iteration of pruning is required
		NumUnprunedRefs = 0;
		int32_t LowestRefCnt = 0;
		int32_t LowestPruneRefIdx = 0;
		int32_t NumRefsYet2Prune = 0;
		int32_t Num4Pruning = 0;
		int32_t Nxt4Pruning = 0;
		for (RefIdx = 0; RefIdx < NumRefIDs; RefIdx++)
			{
			if(pPruneRefs[RefIdx] == 1)	// if already known as pruned
				continue;				// then no further checking for pruning required for this reference
			NumRefsYet2Prune++;
			if (pGenomeRefCnts[RefIdx] <= CurHighRefCntThres) // counts less than threshold are assumed to be just noise - these references are to be pruned in next iteration
				{
				pPruneRefs[RefIdx] = 1;	// marking reference to be pruned if there is a next iteration
				Num4Pruning++;			// counting number of references which will be pruned
				}
			else						// else potentially to be pruned next iteration
				{
				if(pGenomeRefCnts[RefIdx] == LowestRefCnt)		// counting those with same lowest count potentially to be pruned if Num4Pruning == 0
					Nxt4Pruning++;
				if(LowestRefCnt == 0 || pGenomeRefCnts[RefIdx] < LowestRefCnt) // if a new lowest count then 
					{
					LowestRefCnt = pGenomeRefCnts[RefIdx];		// note reference with lowest count and what that count is
					LowestPruneRefIdx = RefIdx;
					Nxt4Pruning = 1;							// number potentially to be pruned if Num4Pruning == 0, incremented if others have same lowest count
					}
				NumUnprunedRefs++;		// if no more pruning in a subsequent iteration then there would be this many references remaining to choose from for grouping
				}
			}

		if (NumRefsYet2Prune <= MinUnprunedRefs)					// ensure not about to excessively prune!
			{
			bPruning = false;
			continue;
			}

		// need to check if all refs are to be pruned - can only mean that there were very few bins, if any, with the current source having alignments
		if (Num4Pruning == NumRefIDs)							
			{	
			gDiagnostics.DiagOut(eDLWarn, gszProcName, "ProcessAlleleScoredBins: Check source '%s' as it has very few, if any, differential alignment scores", LocateReadset(CurSrcID));
			for (RefIdx = 0; RefIdx < NumRefIDs; RefIdx++)		// ensuring all references are marked as being pruned
				pPruneRefs[RefIdx] = 1;	
			bPruning = true;									// ensuring another pruning check iteration will mark each source bin as having references which were fully pruned
			bSourceNoDiffRefs = true;
			continue;						
			}

		if(Num4Pruning > 0)
			{
			if((NumRefsYet2Prune - Num4Pruning) < MaxUnprunedRefs) // avoid potential excessive pruning even if no changes to CurHighRefCntThres
				bPruning = false;							
			continue;										// no changes to CurHighRefCntThres if there is another iteration
			}
 
		if ((NumRefsYet2Prune - Nxt4Pruning) < MaxUnprunedRefs) // avoid potential excessive pruning even if no changes to CurHighRefCntThres
			{
			bPruning = false;
			continue;
			}
			...
			for (RefIdx = 0; RefIdx < NumRefIDs; RefIdx++)
				{
				if(pPruneRefs[RefIdx] == 1)				// already known
					continue;
				if(pGenomeRefCnts[RefIdx] == LowestRefCnt) // any with the LowestRefCnt are to be pruned
					pPruneRefs[RefIdx] = 1;
				}
			CurHighRefCntThres = LowestRefCnt;
			}
		}
	while(bPruning);

	// Finished pruning - identified which references are to be retained as highest scoring targets for current source
	// An issue though, is that it is possible for some source bins to contain unpruned reference targets which were earlier identified as being
	// low scoring (very low alignment rates?) but were not imputed and thus individually marked for pruning.
	// To ensure that these individual bins references marked for pruning still become candidates for later reference assignment then
	// it is forced that there is at least one unpruned reference in any one bin.
	if(!bSourceNoDiffRefs)			// check if there were some differential reference scores
		{
		int64_t TotSrcRetainedRefs = 0;
		for (BinIdx = (int64_t)SrcIdx * NumRefIDs; BinIdx < m_UsedASBins; BinIdx += ((int64_t)NumSrcIDs * NumRefIDs))	// iterating each bin for the current source to first reference bin
			{
			pASBin = &m_pASBins[BinIdx];
			int64_t NumRefsRetained = 0;
			for (RefIdx = 0; RefIdx < NumRefIDs; RefIdx++, pASBin++)
				{
				if (!(pPruneRefs[RefIdx] & 0x01))						// count number of individual bin references which remain unpruned
					if(!(pASBin->ProcState & cBinRefPruned))
						NumRefsRetained++;
				}
			if(NumRefsRetained == 0)										// if no retained references then force at least some ...
				{
				pASBin = &m_pASBins[BinIdx];
				for (RefIdx = 0; RefIdx < NumRefIDs; RefIdx++, pASBin++)
					{
					if (!(pPruneRefs[RefIdx] & 0x01))						// if this reference is to be retained
						{
						pASBin->ProcState &= ~cBinRefPruned;				// forcing that bin ref to be a unpruned (retained) reference
						if(pASBin->ScorePartial == 0.0)						// if no score then give it a pseudo score, pseudo score expected to less (may be exceptions!) than any actual scores recorded for this reference
							pASBin->ScorePartial = 0.0001;					// any bins with actual scores should win and become the highest scoring in subsequent processing	
						if(pASBin->ScoreExact == 0.0)
							pASBin->ScoreExact = 0.0001;
						NumRefsRetained++;
						}
					}
				}
			TotSrcRetainedRefs += NumRefsRetained;
			}
		if(TotSrcRetainedRefs < 1)
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "ProcessAlleleScoredBins: Unexpected - Source ('%s') has no alignments with any references in genome", LocateReadset(CurSrcID));
			delete[]pGenomeRefCnts;
			delete[]pPruneRefs;
			Reset();
...
delete[]pPruneRefs;
	...
	// iterate over bins in complete genome and retain the highest scoring for each ChromID.BinID.SrcID.RefID for those RefID's which were unpruned (!cBinRefPruned)
CurChromID = 0;
CurBinID = 0;
CurBinSize = 0;
CurRefID = 0;
...
for (BinIdx = 0; BinIdx < m_UsedASBins; BinIdx++, pASBin++)
	{
	if (pASBin->ChromID == CurChromID && pASBin->BinID == CurBinID && pASBin->SrcID == CurSrcID)	// if same Chrom.BinID.SrcID then will be iterating over the references 
		{
		if (pASBin->ProcState & cBinSourceNoDiffRefs)						// bin non-differentiated?
			{
			pASBin->ProcState &= ~cBinRefSelected;							// this bin can't be selected reference
			pASBin->ProcState |= cBinRefPruned;								// force to be pruned
			continue;
			}

		if (pASBin->ProcState & cBinRefPruned)								// bin with a pruned reference?
			{
			pASBin->ProcState &= ~cBinRefSelected;							// this bin can't be highest scoring reference, references selected must be unpruned
			continue;
			}

		// bin hasn't been pruned, selecting highest scoring
		if (pASBin->ScorePartial > CurScorePartial || (pASBin->ScorePartial == CurScorePartial && pASBin->ScoreExact > CurScoreExact))
			{
			if(pASBinMH != nullptr)
				pASBinMH->ProcState &= ~cBinRefSelected;
			pASBin->ProcState |= cBinRefSelected;
			CurScorePartial = pASBin->ScorePartial;
			CurScoreExact = pASBin->ScoreExact;
			pASBinMH = pASBin;
			}
		else
			pASBin->ProcState &= ~cBinRefSelected;		// this bin is not the highest scoring
		continue;
		}

	// new ChromID.BinID.SrcID
	// until otherwise proven, selecting initial reference as being the highest scoring CurSrcID vs. reference
	CurChromID = pASBin->ChromID;
	CurBinID = pASBin->BinID;
	CurSrcID = pASBin->SrcID;

	if(pASBin->ProcState & cBinSourceNoDiffRefs)					// if non-differentiated then force to be the selected
		{
		pASBin->ProcState |= cBinRefSelected;						
		pASBin->ProcState &= ~cBinRefPruned;						
		pASBinMH = pASBin;
		continue;
		}

	if (pASBin->ProcState & cBinRefPruned)					// Check if 1st bin in new Chrom.BinID.SrcID is a reference pruned bin
		{
		pASBin->ProcState &= ~cBinRefSelected;					// reference pruned, this bin can't be the highest scoring
		CurScorePartial = -1.0;
		CurScoreExact = -1.0;
		pASBinMH = pASBin;
		continue;
	...
	CurScorePartial = pASBin->ScorePartial;						// 1st bin not reference pruned and not cBinSourceNoDiffRefs
	CurScoreExact = pASBin->ScoreExact;
	pASBin->ProcState |= cBinRefSelected;						// flag that this thus far is the highest scoring bin
	pASBinMH = pASBin;
	}

// the following code section is just doing a sanity check for exactly 1 reference set as being highest scoring and reporting if not exactly 1

int32_t MarkAsPruned = 0;
int32_t MarkAsHighScored = 0;
for (SrcIdx = 0; SrcIdx < NumSrcIDs; SrcIdx++)						// iterating each source
	{
	CurChromID = 0;
	for (BinIdx = (int64_t)SrcIdx * NumRefIDs; BinIdx < m_UsedASBins; BinIdx += ((int64_t)NumSrcIDs * NumRefIDs))	// iterating each bin for the current source
		{
		pASBin = &m_pASBins[BinIdx];
		if(pASBin->ChromID != CurChromID)							// starting bin in a new chromosome?
			CurChromID = pASBin->ChromID;
		CurSrcID = pASBin->SrcID;
		CurBinID = pASBin->BinID;
		MarkAsPruned = 0;
		MarkAsHighScored = 0;
		for (RefIdx = 0; RefIdx < NumRefIDs; RefIdx++, pASBin++)	// iterating each reference for current source and bin
			{
			if(pASBin->ProcState & cBinRefPruned)						// marked as pruned
				MarkAsPruned++;
			if (pASBin->ProcState & cBinRefSelected)					// marked as highest scoring and thus selected reference
				MarkAsHighScored++;
			}
		// expecting exactly 1 marked as cBinRefSelected!!!
		if(MarkAsHighScored != 1)
			gDiagnostics.DiagOut(eDLWarn, gszProcName, "ProcessAlleleScoredBins: %s.%d.%s Unexpected - high scored: %d, number pruned: %d", LocateChrom(CurChromID),CurBinID,LocateReadset(CurSrcID),MarkAsHighScored, MarkAsPruned);
		}
	}

// it is quite common to have outliers, an outlier is being defined as a reference which differs from two immediately bracketing exactly matching references
int32_t ChkOutliers[3];					// selected references for last 3 bins  - same source
int32_t NumOutliersChanged;
for (SrcIdx = 0; SrcIdx < NumSrcIDs; SrcIdx++)						// iterating each source
	{
	NumOutliersChanged = 0;
	CurChromID = 0;
	memset(ChkOutliers,0,sizeof(ChkOutliers));
	for (BinIdx = (int64_t)SrcIdx * NumRefIDs; BinIdx < m_UsedASBins; BinIdx += ((int64_t)NumSrcIDs * NumRefIDs))	// iterating each bin for the current source to first reference bin
		{
		pASBin = &m_pASBins[BinIdx];
		if(pASBin->ChromID != CurChromID)								// a new chrom starting?
			{
			CurChromID = pASBin->ChromID;
			memset(ChkOutliers, 0, sizeof(ChkOutliers));
			pASBinMH = nullptr;
			}
		if (pASBin->ProcState & cBinSourceNoDiffRefs)
			continue;
		if(pASBin->BinID > 1)
			pASBinMH = &pASBin[-((int64_t)NumSrcIDs * NumRefIDs)];
		ChkOutliers[0] = ChkOutliers[1];
		ChkOutliers[1] = ChkOutliers[2];
		for (RefIdx = 0; RefIdx < NumRefIDs; RefIdx++, pASBin++)	// iterating each reference for current source at the current bin looking currently marked highest scoring reference
			{
			if (pASBin->ProcState & cBinRefSelected)							// currently marked as highest scoring reference, check bracketing bins
				{
				ChkOutliers[2] = RefIdx+1;
				if(ChkOutliers[0] == 0)
					break;

				if(ChkOutliers[0] == ChkOutliers[2] && ChkOutliers[1] != ChkOutliers[2])
					{
					pASBin = &pASBinMH[ChkOutliers[1]-1];
					pASBin->ProcState &= ~cBinRefSelected;
					pASBin = &pASBinMH[ChkOutliers[2]-1];
					pASBin->ProcState |= cBinRefSelected;
					ChkOutliers[1] = ChkOutliers[2];
					NumOutliersChanged++;
					}
...
gDiagnostics.DiagOut(eDLInfo, gszProcName, "ProcessAlleleScoredBins: writing founder instances references output file: '%s'", szSrcRefs);
// output header
m_OutBuffIdx = sprintf((char*)m_pszOutBuffer, "\"Chrom\",\"BinID\",\"BinLoci\",\"BinSize\"");
for (RefIdx = 0; RefIdx < NumRefIDs; RefIdx++)
	m_OutBuffIdx += sprintf((char*)&m_pszOutBuffer[m_OutBuffIdx], ",\"%s\"", LocateReadset(m_FndrIDMappings[RefIdx]));
m_OutBuffIdx += sprintf((char*)&m_pszOutBuffer[m_OutBuffIdx], ",\"GrpMembers:0\"");
if (!CUtility::RetryWrites(m_hOutFile, m_pszOutBuffer, m_OutBuffIdx))
	{
	gDiagnostics.DiagOut(eDLFatal, gszProcName, "ProcessAlleleScoreBins: Fatal error in RetryWrites()");
	Reset();
...
m_OutBuffIdx = 0;
	...
	// Note that there is a pseudo reference 'GrpMembers:0' used to capture instances when there was no reference differentials genome wide for a given source

RefIDidx = 0;
GrpIdx = 0;
RefIdx = 0;

pRefIDGrpMbrs = new int32_t[NumRefIDs+1];
memset(pRefIDGrpMbrs, 0, sizeof(int32_t) * (NumRefIDs+1));
pASBin = m_pASBins;
CurBinID = 0;
CurChromID = 0;
CurBinSize = 0;
for (BinIdx = 0; BinIdx < m_UsedASBins; BinIdx++, pASBin++)
	{
	if (CurChromID != pASBin->ChromID || CurBinID != pASBin->BinID)	// starting a new chromosome or new bin
		{
		if (CurChromID > 0)
			{
			m_OutBuffIdx += sprintf((char*)&m_pszOutBuffer[m_OutBuffIdx], "\n\"%s\",%d,%d,%d", LocateChrom(CurChromID), CurBinID, CurBinLoci, CurBinSize);
			for (RefIdx = 0; RefIdx <= NumRefIDs; RefIdx++)
				{
				m_OutBuffIdx += sprintf((char*)&m_pszOutBuffer[m_OutBuffIdx], ",%d", pRefIDGrpMbrs[RefIdx]);
				if (m_OutBuffIdx + 100000 > m_AllocOutBuff)
					{
					if (!CUtility::RetryWrites(m_hOutFile, m_pszOutBuffer, m_OutBuffIdx))
						{
						gDiagnostics.DiagOut(eDLFatal, gszProcName, "ProcessAlleleScoreBins: Fatal error in RetryWrites()");
	...
	if (pASBin->ProcState & cBinRefSelected)
		{
		if(pASBin->ProcState & cBinSourceNoDiffRefs)
			pRefIDGrpMbrs[NumRefIDs] += 1;
		else
...
gDiagnostics.DiagOut(eDLInfo, gszProcName, "ProcessAlleleScoredBins: completed writing founder instances references output file: '%s'", szSrcRefs);
// report the actual highest scored reference for each ChromID.BinLoci.SrcID. Highest scored bin will have ProcState bit cBinRefSelected set
...
gDiagnostics.DiagOut(eDLInfo, gszProcName, "ProcessAlleleScoredBins: created or truncated source to reference highest scoring mappings output file: '%s'", szSrcRefs);
...
for (BinIdx = 0; BinIdx < m_UsedASBins; BinIdx++, pASBin++)
	{
	if (!(pASBin->ProcState & cBinRefSelected))   // only interested in highest scoring bins - a basic assumption is that there will be a highest scoring bin for each source, if not then a major problem as earlier processing should have picked this up!!!!
		continue;
	if (CurChromID != pASBin->ChromID)
		...
		if (CurBinID != pASBin->BinID)	// same chromosome but bin changed
			{
			CurBinID = pASBin->BinID;
			CurBinLoci = pASBin->BinLoci;
			CurBinSize = pASBin->BinSize;
...
gDiagnostics.DiagOut(eDLInfo, gszProcName, "ProcessAlleleScoredBins: completed writing to reference mappings output file: '%s'", szSrcRefs);
...
Rslt = eBSFSuccess; // a big assumption!!!
gDiagnostics.DiagOut(eDLInfo, gszProcName, "ProcessAlleleScoredBins: created or truncated groupings output file: '%s'", szSrcRefs);

	// output header
pASBin = m_pASBins;
m_OutBuffIdx = sprintf((char*)m_pszOutBuffer, "\"ExprID\",\"RowID\",\"Chrom\",\"BinID\",\"Loci\",\"Len\",\"MinDistance\",\"MaxDistance\",\"MaxHaplotypes\",\"ActualDistance\",\"ActualHaplotypes\"");
for (BinIdx = 0; BinIdx < m_NumReadsetTypes[1]; BinIdx++, pASBin += m_NumReadsetTypes[0])
	m_OutBuffIdx += sprintf((char*)&m_pszOutBuffer[m_OutBuffIdx], ",\"%s\"", LocateReadset(pASBin->SrcID));
m_OutBuffIdx += sprintf((char*)&m_pszOutBuffer[m_OutBuffIdx], ",\"GrpMembers:1\",\"GrpMembers:2\",\"GrpMembers:3\",\"GrpMembers:4\",\"GrpMembers:5\",\"GrpMembers:0\"");
...
for (BinIdx = 0; BinIdx < m_UsedASBins; BinIdx++, pASBin++)
	{
	if (!(pASBin->ProcState & cBinRefSelected))   // only interested in highest scoring bins - a basic assumption is that there will be a single highest scoring bin for each source, if not then a major problem as earlier processing should have picked this up!!!!
		continue;
	if (CurChromID != pASBin->ChromID || CurBinID != pASBin->BinID || CurBinSize != pASBin->BinSize)
		{
		if (CurChromID != 0)
			for (GrpIdx = 0; GrpIdx < 6; GrpIdx++)
		...
		m_OutBuffIdx += sprintf((char*)&m_pszOutBuffer[m_OutBuffIdx], "\n1,%d,\"%s\",%d,%d,%d", pMRAASRefGrp->AllocID, LocateChrom(CurChromID), CurBinID, CurBinLoci, CurBinSize);
	...
	if (pASBin->ProcState & cBinSourceNoDiffRefs)													// if can't differentiate then
		{
		CurRefGrp = 0;																				// these are in GrpMembers:0
		GrpInstDist[5]++;
		}
	else
		{
		if ((pASRefIDGrp = LocateASRefIDGrp(CurChromID, CurBinLoci, pASBin->RefID)) == nullptr)		// shouldn't ever, but being sure!!!!
			CurRefGrp = 5;																			// throw these into the outlier group 5
		else
...
gDiagnostics.DiagOut(eDLInfo, gszProcName, "ProcessAlleleScoredBins: completed groupings output file: '%s'", szSrcRefs);
...
// sorting tsCHChromScores by ChromID.BinID.SrcID.RefID ascending
int
CCallHaplotypes::SortChromScores(const void* arg1, const void* arg2)
{
	tsCHChromScores* pEl1 = (tsCHChromScores*)arg1;
	tsCHChromScores* pEl2 = (tsCHChromScores*)arg2;
	if (pEl1->ChromID < pEl2->ChromID)					// sort ascending
		return(-1);
	if (pEl1->ChromID > pEl2->ChromID)
		return(1);

	if (pEl1->BinID < pEl2->BinID)
		return(-1);
	if (pEl1->BinID > pEl2->BinID)
		return(1);
...
// sorting tsASBins by ChromID.BinID.SrcID.RefID ascending
int
CCallHaplotypes::SortASBins(const void* arg1, const void* arg2)
{
