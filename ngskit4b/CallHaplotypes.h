...
class CCallHaplotypes
{
	...
	int
		GenKMerGrpHammings(int32_t KMerSize,		// use this KMer sized sequences when identifying group segregation hammings
			...
			int32_t KMerCoverage,					// if > 0 then allow upto this number of bp within a KMer to have no coverage, maximum is 50% of KMerSize
	...
	int32_t				// < 0 error, otherwise the total number of non-consensus PBA over all pPBAs: if 0 then all PBAs of ConsensusLen were identical to each other
		GenGrpConsensus(int32_t ConsensusLen,		// PBAs consensus length
