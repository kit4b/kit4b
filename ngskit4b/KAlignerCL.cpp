...
Because of the potential for confusion by users unaware of functionalityand process parameterisation changes then the modified source base
...
// KAlignerCL.cpp : Defines the entry point for the console application.
...
int
KProcess(etPMode PMode,					// processing mode
...
char szStatsFile[_MAX_PATH];			// optional output various basic distribution counts/stats to files named with this prefix
...
struct arg_int *maxsubs = arg_int0("s","substitutions","<int>",	"accept up to this number of aligner induced substitutions per 100bp of individual read length (default is 5, range 0..15)");
...
struct arg_str *experimentname = arg_str0("w","experimentname","<str>",		"experiment name");
struct arg_str *experimentdescr = arg_str0("W","experimentdescr","<str>",	"experiment description");
...
if (!argerrors)
	...
	FMode = (etFMode)(format->count ? format->ival[0] : 0);
	if (FMode < eFMsam || FMode > eFMbed)
		{
	...
	if(pe2inputfiles->count > 0 && PEproc == ePEdefault)	// if PE2 files specified then must also have PEproc mode specified
		{
		PEproc = ePEunique;
		gDiagnostics.DiagOutMsgOnly(eDLWarn,"PE2 file(s) with '-u<files>' specified, defaulting PE processing mode to unique alignments only '-U2'\n");
		}
	...
	if(statsfile->count)
		...
		if(PEproc == ePEdefault && FMode == eFMsamAll)
			{
			gDiagnostics.DiagOut(eDLFatal,gszProcName,"Error: Output statistics file '-O<file>' not available in '-M%d' output mode\n",eFMsamAll);
			exit(1);
			}
	...
	if(FMode == eFMbed)
		...
		if(szTrackTitle[0] == '\0')
			{
			gDiagnostics.DiagOut(eDLWarn,gszProcName,"Warning: output format requested to be UCSC BED but no track title with '-t<title' specified, defaulting to 'ngskit4b kalign'\n");
			strcpy(szTrackTitle,"ngskit4b kalign");
			}
		}
...
// show user current resource limits
	...
	if(szExperimentName[0] == '\0')
		strcpy(szExperimentName,"Unspecified");
...
#endif
	gStopWatch.Start();
	Rslt = KProcess((etPMode)PMode,szExperimentName,SampleNthRawRead,(etFQMethod)Quality,bSOLiD,bBisulfite,(etPEproc)PEproc,PairMinLen,PairMaxLen,bPairStrand,bPEcircularised,bPEInsertLenDist,
...
char szRsltsFile[_MAX_PATH];			// results to this file
char szTargFile[_MAX_PATH];				// align against this target suffix array genome file
char szEmpty[1];						// dummy empty string
int NumPE1InputFiles;					// number of input PE1 or single ended file spe
char *pszPE1InputFiles[cMaxInFileSpecs];		// names of input files (wildcards allowed unless processing paired ends) containing raw reads
int NumPE2InputFiles;					// number of input PE2 file specs
char *pszPE2InputFiles[cMaxInFileSpecs];		// optional raw paired reads are in these files
...
struct arg_int *qual = arg_int0("g","quality","<int>",		    "fastq quality scoring - 0 - Sanger or Illumina 1.8+, 1 = Illumina 1.3+, 2 = Solexa < 1.3, 3 = Ignore quality (default = 3)");
struct arg_file *sfxfile = arg_file1("I","sfx","<file>",		"align against this suffix array (kit4b index generated) file");
struct arg_file *outfile = arg_file1("o","out","<file>",		"output packed base alleles to this file");
...
struct arg_str *experimentid = arg_str1("w","experimentid","<str>",		"experiment identifier");
struct arg_str *readsetid = arg_str1("W","readsetid","<str>",			"readset identifier");
...
if (!argerrors)
	...
	szExperimentID[0] = '\0';
	if(experimentid->count)
		{
		strncpy(szExperimentID,experimentid->sval[0],sizeof(szExperimentID));
		szExperimentID[sizeof(szExperimentID)-1] = '\0';
		CUtility::TrimQuotedWhitespcExtd(szExperimentID);
		CUtility::ReduceWhitespace(szExperimentID);
		}
	else
		szExperimentID[0] = '\0';
	if(szExperimentID[0] == '\0')
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"No experiment identifier specified");
		exit(1);
		}
	...
	if(readsetid->count)
		{
		strncpy(szReadsetID,readsetid->sval[0],sizeof(szReadsetID));
		szReadsetID[sizeof(szReadsetID)-1] = '\0';
		CUtility::TrimQuotedWhitespcExtd(szReadsetID);
		CUtility::ReduceWhitespace(szReadsetID);
		}
	else
		szReadsetID[0] = '\0';
	if(szReadsetID[0] == '\0')
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"No readset identifier specified");
		exit(1);
		}
	...
	if(abs(MinChimericLen) > 0 && bSOLiD)		// chimeric read processing not currently supported for SOLiD colorspace processing
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"Error: Sorry, chimeric read processing not supported in this release if SOLiD");
		exit(1);
		}
	...
	if(szTrackTitle[0] == '\0')
		strcpy(szTrackTitle,"ngskit4b genpba");
...
// show user current resource limits
	...
	gDiagnostics.DiagOutMsgOnly(eDLInfo,"Experiment identifier : '%s'",szExperimentID);
	gDiagnostics.DiagOutMsgOnly(eDLInfo,"Readset identifier : '%s'",szReadsetID);
	gDiagnostics.DiagOutMsgOnly(eDLInfo,"Raw read or paired reads sampling is : every %u",SampleNthRawRead);
	gDiagnostics.DiagOutMsgOnly(eDLInfo,"Processing in %s mode",bSOLiD ? "colorspace (SOLiD)" : "standard basespace");
	...
	pszDescr = "Packed Base Alleles only";
	...
	Rslt = KProcess((etPMode)PMode,szExperimentID,SampleNthRawRead,(etFQMethod)Quality,bSOLiD,bBisulfite,(etPEproc)PEproc,PairMinLen,PairMaxLen,bPairStrand,false,false,
					...
					szEmpty,szEmpty,szEmpty,szEmpty,szEmpty,szContamFile,NumIncludeChroms,pszIncludeChroms,NumExcludeChroms,pszExcludeChroms);
	Rslt = Rslt >=0 ? 0 : 1;
	if(gExperimentID > 0)
		{
		if(gProcessingID)
...
Rslt = pKAligner->Align(PMode,			// processing mode
			pszExperimentName,			// labels this experiment
