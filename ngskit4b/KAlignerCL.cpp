...
Because of the potentential for confusion by users unaware of functionalityand process parameterisation changes then the modified source base
...
struct arg_int *pmode = arg_int0("m","mode","<int>",		    "alignment processing mode: 0 - standard sensitivity, 1 - more sensitive (slower), 2 - ultra sensitive (slowest), 3 - less sensitive (quicker)");
				...
				// NOTE: support for CSV format out has been dropped from the command line parameterisations
				//		but is still available in the KAligner source body in case CSV support is restored at some future release
#ifdef _USECSVOUTFORMAT
...
#else
struct arg_int* format = arg_int0("M", "format","<int>",		"output format: 0 - SAM (BAM) format with accepted aligned, 1 - SAM (BAM) format with all reads, 2 - UCSC BED (default is 1)");
#endif
...
if (!argerrors)
	...
	#else
	// mapping 0 (was eFMdefault) to be eFMsam
	//         1 ( was eFMmatch) to eFMsamAll
	// ........2 ( was eFMread) to eFMbed

	FMode = (etFMode)(format->count ? format->ival[0] : 0);
	if (FMode < eFMdefault || FMode > eFMread)
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "Error: Output format mode '-m%d' specified outside of range %d..%d\n", FMode, eFMdefault, eFMread);
		exit(1);
		}
	switch (FMode) {
		case eFMdefault:
	...
	if(FMode == eFMbed)
		...
		if(szTrackTitle[0] == '\0')
			{
			gDiagnostics.DiagOut(eDLWarn,gszProcName,"Warning: output format requested to be UCSC BED but no track title with '-t<title' specified, defaulting to 'kit4b align'\n");
			strcpy(szTrackTitle,"kit4b align");
			}
		}
...
#endif
	gStopWatch.Start();
	Rslt = KProcess((etPMode)PMode,SampleNthRawRead,(etFQMethod)Quality,bSOLiD,bBisulfite,(etPEproc)PEproc,PairMinLen,PairMaxLen,bPairStrand,bPEcircularised,bPEInsertLenDist,
