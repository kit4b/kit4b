...
if (!m_bPEProc)
	gDiagnostics.DiagOut(eDLInfo, gszProcName, "(Instance %d) Total of %zu SE reads processed", ProcessingID, TotNumSEReads);
else
	gDiagnostics.DiagOut(eDLInfo, gszProcName, "(Instance %d) Total of %zu reads, %zu PE pairs processed", ProcessingID, TotNumSEReads, TotNumPEReads);

gDiagnostics.DiagOut(eDLInfo, gszProcName, "(Instance %d) Total of %zu reads not accepted for processing as they were underlength", ProcessingID, NotProcUL);
...
gDiagnostics.DiagOut(eDLInfo, gszProcName, "(Instance %d) Total of %zu reads containing 'N's and/or %zu below minimum Phred scores not accepted for duplicate processing", ProcessingID, NotProcNs,NotProcQS);
...
for (int Idx = 0; Idx < 100; Idx++)
#ifdef _WIN32
	BuffIdx += sprintf(&szRptBuff[BuffIdx],"\n%1.2f,%zd",(double)Idx*0.01,m_ProbNoReadErrDist[Idx]);
#else
...
if (!m_bPEProc)								// if SE then process each input reads file separately
	...
	for (Idx = 0; Idx < m_NumPE1InFiles; Idx++, pInPE1File++)
		...
		if (Rslt == 0)
			...
			gDiagnostics.DiagOut(eDLInfo, gszProcName, "(Instance %d) Thread %d: Unable to accept - %d were underlength, %d contained 'N's, %zd below minimum Phred - reads from input reads file", pThread->ProcessingID,
	...
	gDiagnostics.DiagOut(eDLInfo, gszProcName, "(Instance %d) Thread %d: Unable to accept - %zd were underlength, %zd contained 'N's, %zd below minimum Phred - total reads from input reads files", pThread->ProcessingID,
