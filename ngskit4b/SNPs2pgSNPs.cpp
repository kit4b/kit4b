...
SNPs2pgSNPs(int argc, char** argv)
{
	...
	struct arg_dbl* locerrrate = arg_dbl0("p", "locerrrate", "<dbl>", "default local sequencing error rate in a 100bp window centered around the putative SNP site (default 0.02, range 0.001 .. 0.25)");
	struct arg_dbl* pvaluethres = arg_dbl0("p", "pvalue", "<dbl>", "SNP PValue threshold (default 0.05, range 0.005..0.25)");
	struct arg_dbl* minalleleprop = arg_dbl0("P", "minalleleprop", "<dbl>", "SNP minimum allele proportion of loci site coverage (default 0.05, range 0.01..0.99)");
	...
	void* argtable[] = { help,version,FileLogLevel,LogFile,
						pmode,locerrrate,pvaluethres,minalleleprop,mincoverage,insnps,setop,seta,setb,outpgsnps,experimentdescr,assemblyname,trackname,end };
	...
	if (!argerrors)
	{
		...
		MinAlleleProp = minalleleprop->count ? minalleleprop->dval[0] : 0.05;
...
Rslt = pSNPs2pgSNPs->Process(Mode, LocalSeqErrRate,MinCoverage, MinAlleleProp,PValueThres, pszTrackName, pszAssemblyName,pszExperimentDescr, SetOp, NumInSetA, ppszSetA,NumInSetB, ppszSetB, pszSNPFile, pszOutFile);
...
while((Rslt=m_pCSV->NextLine()) > 0)				// onto next line containing fields
	...
	if(!ExpNumFields)
		...
		if(((m_NumCultivars * 9) + 4) != NumFields)
			{
			gDiagnostics.DiagOut(eDLFatal,gszProcName,"Invalid number (%d) of fields parsed from '%s' near line %d, is this file generated by 'ngskit4b align/snpmarkers'?",NumFields, m_szInSNPsFile,RowNumber);
			Reset();
			return(eBSFerrFieldCnt);
			}
	...
	gDiagnostics.DiagOut(eDLInfo, gszProcName, "Evaluating SNP alleles at %s:%d", pSNPSite->szChrom, pSNPSite->SNPLoci);
	...
	for (CultivarIdx = 0; CultivarIdx < m_NumCultivars; CultivarIdx++, pCultivar++)
		...
		switch(m_SetOp) {
			...
			case eSOCplUnion:				// complement of union SetA and SetB
				if(pCultivar->flgInSetA || pCultivar->flgInSetB) 
					continue;
		...
		for (int Idx = 0; Idx < 4; Idx++)
			...
			PValue = 1.0 - Stats.Binomial(pCultivar->TotBaseCnts, pCultivar->BaseCnts[Idx], pCultivar->Score);
			if(PValue > m_PValueThres)
				continue;

			pSNPSite->BaseCnts[Idx].Cnts += 1;
			if(pSNPSite->BaseCnts[Idx].QScore < PValue)
	...
	pSNPSite->SNPId = ++m_NumSNPs;		// accepted SNP
...
if(bHeader)
	...
	switch (m_ReportFormat) {
		case eRMFpgSNP:						// report in pgSNP format
			...
			if(!stricmp(pSNPSite->szChrom, "NC_045512.2"))
				strcpy(pSNPSite->szChrom, "NC_045512v2");
			...
			m_LineBuffOffs += sprintf(&m_pszLineBuff[m_LineBuffOffs], "browser position %s:%d-%d", pSNPSite->szChrom, StartLoci, EndLoci);
			break;
	...
	switch (m_ReportFormat) {
		case eRMFpgSNP:						// report in pgSNP format
			...
			if(!stricmp(pSNPSite->szChrom, "NC_045512.2"))
				strcpy(pSNPSite->szChrom, "NC_045512v2");
			m_LineBuffOffs += sprintf(&m_pszLineBuff[m_LineBuffOffs], "\n%s\t%d\t%d\t", pSNPSite->szChrom, pSNPSite->SNPLoci, pSNPSite->SNPLoci + 1);
			NumAlleles = 0;
			for (int Idx = 0; Idx < 4; Idx++)
				{
		...
		case eRMFvcf:						// report in VCF 4.1 format
			...
			SumQScores = 0;
			Depth = 0;
			m_LineBuffOffs += sprintf(&m_pszLineBuff[m_LineBuffOffs], "%s\t%u\tSNP%d\t%c\t", pSNPSite->szChrom, pSNPSite->SNPLoci + 1, pSNPSite->SNPId, CSeqTrans::MapBase2Ascii(pSNPSite->RefBase));
			for (int Idx = 0; Idx < 4; Idx++)
				{
