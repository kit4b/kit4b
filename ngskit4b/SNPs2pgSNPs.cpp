...
SNPs2pgSNPs(int argc, char** argv)
{
	...
	void* argtable[] = { help,version,FileLogLevel,LogFile,
						pmode,pvaluethres,minalleleprop,mincoverage,insnps,outpgsnps,experimentdescr,assemblyname,trackname,end };
	...
	if (!argerrors)
	{
		...
		if (szOutpgSNPsFile[0] == '\0')
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "No UCSC Personal Genome SNP format output file specified");
			exit(1);
			}
		...
		gDiagnostics.DiagOutMsgOnly(eDLInfo, "SNP to UCSC Personal Genome SNP format conversion : '%s'", pszDescr);
		...
		gDiagnostics.DiagOutMsgOnly(eDLInfo, "Output UCSC Personal Genome SNP format file : '%s'", szOutpgSNPsFile);
...
Rslt = pSNPs2pgSNPs->Process(Mode, MinCoverage, MinAlleleProp,PValueThres, pszTrackName, pszAssemblyName,pszExperimentDescr, pszSNPFile, pszOutFile);
...
while((Rslt=m_pCSV->NextLine()) > 0)				// onto next line containing fields
	...
	for(CultivarIdx = 0; CultivarIdx < m_NumCultivars; CultivarIdx++,pCultivar++)
		...
		if(*pszTxt == 'I' || *pszTxt == 'i')
			pCultivar->bSNPInferenced = true;
		else
			pCultivar->bSNPInferenced = false;
...
strcpy(m_TargAssemblyName, pszAssemblyName);
