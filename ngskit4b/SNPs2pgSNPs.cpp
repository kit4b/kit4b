...
SNPs2pgSNPs(int argc, char** argv)
{
	...
	void* argtable[] = { help,version,FileLogLevel,LogFile,
						pmode,allowinferenced,locerrrate,pvaluethres,minalleleprop,mincoverage,insnps,setop,seta,setb,outpgsnps,experimentdescr,assemblyname,trackname,end };
...
Rslt = pSNPs2pgSNPs->Process(Mode, bAllowInferenced, LocalSeqErrRate,MinCoverage, MinAlleleProp,PValueThres, pszTrackName, pszAssemblyName,pszExperimentDescr, SetOp, NumInSetA, ppszSetA,NumInSetB, ppszSetB, pszSNPFile, pszOutFile);
...
CSNPs2pgSNPs::ProcessSnpmarkersSNPs(void)
{
	...
	while((Rslt = m_pCSV->NextLine()) > 0)				// onto next line containing fields
		...
		if(NumFields == cAlignNumSSNPfields || NumFields < cSSNPMarkerNumFields)
		{
...
CSNPs2pgSNPs::ProcessKalignSNPs(void)
{
	...
	while ((Rslt = m_pCSV->NextLine()) > 0)				// onto next line containing fields
	{
		...
		if (!ExpNumFields)
			{
			if (NumFields != cAlignNumSSNPfields)								// must be exactly this many if 'ngskit4b kalign' format
				{
