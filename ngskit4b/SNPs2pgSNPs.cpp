...
while((Rslt = m_pCSV->NextLine()) > 0)				// onto next line containing fields
	...
	// kalign generates a number of SNP CSV formats - SNP, DiSNP and TriSNPs each with differing numbers of fields
	if (!ExpNumFields)
		{
		switch(NumFields) {
				case cAlignNumSSNPfields:			// if generated by 'kit4b kalign' then there will be this many CSV fields if simple SNP
				case cAlignNumDiSNPfields:			// if generated by 'kit4b kalign' then there will be this many CSV fields if DiSNP localised haplotype
				case cAlignNumTriSNPfields:			// if generated by 'kit4b kalign' then there will be this many CSV fields if TriSNP localised haplotype
				case cAlignNumSSNPXfields:          // if generated by 'kit4b kalign' for all 3 read counts frame shifts (3*4^3) + fixed reference frame shifts (3*3) then there will be this many CSV fields
				case cSSNPMarkerNumFields:		    // if generated by 'kit4b snpmarkers' then there will be this number of CSVfields
					break;
				default:
					gDiagnostics.DiagOut(eDLFatal, gszProcName, "%d fields parsed from '%s' near line %d, was file generated by 'kit4b kalign'?", NumFields, m_szInSNPsFile, RowNumber);
					Reset();
					return(eBSFerrFieldCnt);
				}
		ExpNumFields = NumFields;
	...
	if (!NumSNPsParsed) // an assumption here is that a given SNP file only contains SNPs against one target assembly
		{
		// parse out target assembly name against which alignments were made and the cultivar names
		char* pSrc;
		char* pDst;
	...
	switch(NumFields) {
		...
		case cSSNPMarkerNumFields:		    // if generated by 'kit4b snpmarkers' then there will be this number of CSVfields
			...
			if((pSNPSite->RefBase = CSeqTrans::MapAscii2Base(*pszTxt)) > eBaseN)
				{
		...
		case cAlignNumDiSNPfields:			// if generated by 'kit4b kalign' then there will be this many CSV fields if DiSNP localised haplotype
			SNPlen = 1;  // currently a simplified view for localised haplotypes to see where they aligning in IGV
			m_pCSV->GetUint(19, &pSNPSite->TotBaseCnts);		// get total "Bases" count, can be less than coverage as read alignments must be overlapping all two SNPs
			m_pCSV->GetText(6, &pszTxt);		// get "RefBase"
			if((pSNPSite->RefBase = CSeqTrans::MapAscii2Base(*pszTxt)) > eBaseN)
				{
				gDiagnostics.DiagOut(eDLFatal, gszProcName, "Expected SNP RefBase ('%s') to be one of 'ACGTN' in CSV file '%s' near line %d", pszTxt, m_szInSNPsFile, RowNumber);
				Reset();
			...
			m_pCSV->GetUint(7, &pSNPSite->BaseCnts[0].Cnts);	// get "MMBaseA"
			m_pCSV->GetUint(8, &pSNPSite->BaseCnts[1].Cnts);	// get "MMBaseC"
			m_pCSV->GetUint(9, &pSNPSite->BaseCnts[2].Cnts);	// get "MMBaseG"
			m_pCSV->GetUint(10, &pSNPSite->BaseCnts[3].Cnts);	// get "MMBaseT"
			m_pCSV->GetUint(11, &pSNPSite->BaseCnts[4].Cnts);	// get "MMBaseN"
			pSNPSite->TotMismatches = pSNPSite->TotBaseCnts - pSNPSite->BaseCnts[pSNPSite->RefBase].Cnts;		// derive "Mismatches"
			pSNPSite->LocalSeqErrRate = 0.01;
			break;

		case cAlignNumTriSNPfields:			// if generated by 'kit4b kalign' then there will be this many CSV fields if TriSNP localised haplotype	
			SNPlen = 1;  // currently a simplified view for localised haplotypes to see where they aligning in IGV
			m_pCSV->GetUint(26, &pSNPSite->TotBaseCnts);		// get total "Bases" count, can be less than coverage as read alignments must be overlapping all three SNPs
			m_pCSV->GetText(6, &pszTxt);		// get "RefBase"
			if((pSNPSite->RefBase = CSeqTrans::MapAscii2Base(*pszTxt)) > eBaseN)
				{
				gDiagnostics.DiagOut(eDLFatal, gszProcName, "Expected SNP RefBase ('%s') to be one of 'ACGTN' in CSV file '%s' near line %d", pszTxt, m_szInSNPsFile, RowNumber);
	...
	pSNPSite->BaseCnts[pSNPSite->RefBase].Cnts = CntRef; // initial releases of ngskit4b were returning the reference allele count as being 0
