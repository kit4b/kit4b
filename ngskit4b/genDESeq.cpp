...
// Purpose -
// Loads the output generated by maploci2features processing with '-O<featuremapfile>' and combines these into a tab delimited form ready
// for processing in 'R' by DESeq
// Alternatively, if alignments of RNA-seq reads were directly to a transcriptome with a summary stats file generated then the
// generated AlignCntsDist.csv files can be used directly without having to map alignments to features
//
#include "stdafx.h"
...
bool gbActivity;						// used to determine if activity messages via printf's can be used - output activity if eDLInfo or less
...
struct arg_file *incontrolfiles = arg_filen("i","control","<file>",0,cMaxInFileSpecs,"input control from these files, wildcards allowed");
struct arg_file *inexperfiles = arg_filen("I","experiment","<file>",0,cMaxInFileSpecs,"input experiment from these files, wildcards allowed (optional)");
...
while((Rslt=m_pCSVFile->NextLine()) > 0)	// onto next line containing fields
	...
	if (NumLines == 1)
		...
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "Expected header line as the 1st line with 1st field titled as 'FeatID' in %s", pszInFile);
		delete m_pCSVFile;
		return(eBSFerrFieldCnt);
		}
	if (NumFields != ExpNumFields)
		{
		CSVLineNum = m_pCSVFile->GetLineNumber();
		gDiagnostics.DiagOut(eDLFatal, gszProcName, "Header line had %d fields, line %d has %d fields in '%s'", ExpNumFields, CSVLineNum, NumFields, pszInFile);
		delete m_pCSVFile;
		return(eBSFerrFieldCnt);
		}
	...
	if(ExpNumFields >= 13 && ExpNumFields != 71)
		...
		{
		m_pCSVFile->GetInt(4, &TmpCnt);
		ExonIntronCnts = TmpCnt;
		if(ExpNumFields >= 5)
			m_pCSVFile->GetDouble(5, &RPKM);
		}

	if(MinExonIntronCnts > 0 && ExonIntronCnts < MinExonIntronCnts)
		{
		NumUnderCnts += 1;
		continue;
		}
