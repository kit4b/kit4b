...
if(bProcSepKMers)
	...
	*pSeq++ = eBaseEOG;		// start with this marker so subsequent crick strand processing functions 'know' when all sequence bases processed
	...
	for(ChromIdx = 0; ChromIdx < m_NumGChroms; ChromIdx++, pChrom++)
		...
		gDiagnostics.DiagOut(eDLInfo,gszProcName,"Targeted suffix genome has %d sequences containing %zu total nucleotides, %u sequences of at least K-mer length %d",
														m_NumGChroms,(uint64_t)m_TotAllocHammings,m_NumSubSeqs,m_SubSeqLen);


	gDiagnostics.DiagOut(eDLInfo,gszProcName,"Genome containing %zu total nucleotides loaded with %u subsequences of K-mer length %d...",m_TotAllocHammings,m_NumSubSeqs,m_SubSeqLen);
	}
else
	{
	// determine the total number of subsequences over which Hammings are required
...
pthread_mutex_unlock(&m_hMtxThreadParams);
#endif
gDiagnostics.DiagOut(eDLInfo,gszProcName,"Progress: %zu K-mers processed from %d sequences",PrevApproxNumHammings,PrevApproxNumChroms);
...
for(ThreadIdx = 0; ThreadIdx < m_NumProcThreads; ThreadIdx++)
	...
	while(WAIT_TIMEOUT == WaitForSingleObject( WorkerThreads[ThreadIdx].threadHandle, 60000))
		...
		if(ApproxNumChroms > PrevApproxNumChroms || ApproxNumHammings > PrevApproxNumHammings)
			gDiagnostics.DiagOut(eDLInfo,gszProcName,"Progress: %zu K-mers processed from %d sequences",ApproxNumHammings,ApproxNumChroms);
...
for(SeqIdx = 0; SeqIdx <= (uint32_t)(RHamm+1); SeqIdx++)
	gDiagnostics.DiagOutMsgOnly(eDLInfo,"%d%c,%zu,%1.3f",SeqIdx,SeqIdx == RHamm+1 ? '+' : ' ',CntHist[SeqIdx],(CntHist[SeqIdx]*100.0f)/NumSampled);
...
gDiagnostics.DiagOut(eDLInfo,gszProcName,"Genome containing %zu total nucleotides loaded with %u subsequences of length %d...",TotLen,m_NumSubSeqs,m_SubSeqLen);
