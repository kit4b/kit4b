...
int Process(eGTHMode PMode,			        // 0 if PBA->fasta, 1 if Fasta->PBA
		char* pszSampleID,
	    char* pszGroup,
				   char* pszInFile,	    // input file to convert
				   char* pszOutFile);	// output converted to this file

#ifdef _WIN32
int pbafasta(int argc, char* argv[])
{
	// determine my process name
_splitpath(argv[0], NULL, NULL, gszProcName, NULL);
#else
int
pbafasta(int argc, char** argv)
{
	// determine my process name
...
eGTHMode PMode;			        // 0 if PBA->fasta, 1 if Fasta->PBA

char szInFile[_MAX_PATH];		// input file to convert
...
struct arg_file* infile = arg_file1("i", "infile", "<file>", "process this input file");
struct arg_file* outfile = arg_file1("o", "out", "<file>", "output to this file");
struct arg_end* end = arg_end(200);
void* argtable[] = {help,version,FileLogLevel,LogFile,
					pmode,refassemb,infile,outfile,end};
char** pAllArgs;
int argerrors;
...
// show user current resource limits
    ...
    gDiagnostics.DiagOutMsgOnly(eDLInfo, "Input binned haplotype file : '%s'", szInFile);
...
Rslt = pHaps2Genotype->Process(PMode, pszSampleID, pszGroup, pszInFile, pszOutFile);
...
int 
CHaps2Genotype::Process(eGTHMode PMode,			        // 0 if PBA->fasta, 1 if Fasta->PBA
		char* pszSampleID,
	    char* pszGroup,
				   char* pszInFile,	    // input file to convert
				   char* pszOutFile)	// output converted to this file
{
int Rslt;
Rslt = PopulateGenotypes(1, &pszInFile);
