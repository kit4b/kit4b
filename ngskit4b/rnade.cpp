...
// kit4bde.cpp : Defines the entry point for the console application.
// Purpose -
...
struct arg_int *ftype = arg_int0("t","filetype","<int>",		"input element file format: 0 - auto, 1 - CSV, 2 - BED, 3 - SAM/BAM (default = 0)");
...
// show user current resource limits
	...
	if(NormCntsScale == 0.0)
		...
		switch(FType) {
		case 0:
			gDiagnostics.DiagOutMsgOnly(eDLInfo,"Auto-classify input element file as either CSV, BED or SAM/BAM");
			break;
		...
		case 3:
			gDiagnostics.DiagOutMsgOnly(eDLInfo,"Expecting input element file to be SAM/BAM format");
			break;
		}
...
m_AllocdCtrlAlignReadsMem = 0;
m_AllocdExprAlignReadsLoci = 0;
m_AllocdExprAlignReadsMem = 0;
...
for(ThreadInst = 1; ThreadInst <= (uint32_t)m_NumDEThreads; ThreadInst++,pThreadInst++,
...
uint32_t NumFeaturesProcessed = 0;
...
if(RelLoci >= (uint32_t)pThreadInst->CurRegionLen) // should always start within region but best to check it does!
	return(eBSFerrInternal);
...
if(!bSense)				// if was aligned antisense
	...
	if(ControlCnts)
		{
		if(MeanControlReadLen <= (uint32_t)CurLoci + 1)
		...
		if(ExperimentCnts)
			{
			if(MeanExperimentReadLen <= (uint32_t)CurLoci + 1)
	...
	for(CurBinIdx = 0; CurBinIdx < m_NumBins; CurBinIdx++,pCovAlignBin++)
		if(CurLoci <= (int32_t)pCovAlignBin->BinRelEndLoci)
			break;
	}
...
if(bIsExperiment)
	...
	if(m_CurNumExprAlignReadsLoci >= m_AllocdExprAlignReadsLoci)
		{
		memreq = m_AllocdExprAlignReadsMem + ((size_t)cAlignReadsLociRealloc * sizeof(tsAlignReadLoci));
	#ifdef _WIN32
		pTmpAlloc = (uint8_t *) realloc(m_pExprAlignReadLoci,memreq);
	#else
		...
		m_AllocdExprAlignReadsMem = memreq;
		}
	...
	if(m_CurNumCtrlAlignReadsLoci >= m_AllocdCtrlAlignReadsLoci)
		{
		memreq = m_AllocdCtrlAlignReadsMem + ((size_t)cAlignReadsLociRealloc * sizeof(tsAlignReadLoci));;
	#ifdef _WIN32
		pTmpAlloc = (uint8_t *) realloc(m_pCtrlAlignReadLoci,memreq);
	#else
		...
		m_AllocdCtrlAlignReadsMem = memreq;
		}
	}
...
if(bExperiment)
	...
	if((m_AllocdExprAlignReadsLoci - NumAlignReadsLoci)  > 1000000)
		{
		memreq = (NumAlignReadsLoci + 10000) * sizeof(tsAlignReadLoci);
	#ifdef _WIN32
		pTmpAlloc = (uint8_t *) realloc(m_pExprAlignReadLoci,memreq);
	#else
		pTmpAlloc = (uint8_t *)mremap(m_pExprAlignReadLoci, m_AllocdExprAlignReadsMem,memreq,MREMAP_MAYMOVE);
		...
		m_pExprAlignReadLoci = (tsAlignReadLoci *)pTmpAlloc;
		m_AllocdExprAlignReadsLoci = NumAlignReadsLoci + 10000;
		}
	...
	if((m_AllocdCtrlAlignReadsLoci - NumAlignReadsLoci)  > 1000000)
		...
		m_pCtrlAlignReadLoci = (tsAlignReadLoci *)pTmpAlloc;
		m_AllocdCtrlAlignReadsLoci = NumAlignReadsLoci + 1;
		}
...
for(Idx = 0; Idx < CurNumAlignReadsLoci; Idx++,pCurLoci++)
	...
	pCurLoci->NormCnts = (uint32_t)NormCnts;
...
for(Idx = 0; Idx < NumAlignReadsLoci; Idx++,pTargLoci++)
	...
	if(pTargLoci->NormCnts <= (uint32_t)ArtifactCntsThres)
		continue;

	WinCnts = 0;
	NumCntLoci = 0;
	if((Idx - ChromRelIdx) >= (uint32_t)FlankLen)
		WinIdx = Idx - FlankLen;
	else
		WinIdx = ChromRelIdx;
	while(WinIdx < Idx)
	...
	if(Idx != NumAlignReadsLoci - 1)
		...
		if (NumCntLoci >= 1)
		{
			...
			if (pTargLoci->NormCnts > (uint32_t)MeanX3)
				NormCnts = (uint32_t)(MeanX3 + sqrt((double)pTargLoci->NormCnts - MeanX3));
			else
				continue;
		}
		else
			NormCnts = (uint32_t)(ArtifactCntsThres + sqrt((double)pTargLoci->NormCnts - ArtifactCntsThres));
...
if((Rslt = pBAMfile->Open(pszInFile)) != eBSFSuccess)
	{
...
NumProcessed = 0;
NumRdsExcluded = 0;
while((LineLen = pBAMfile->GetNxtSAMline(szLine)) > 0)
	{
	if(m_LimitAligned > 0 && (uint32_t)NumProcessed > m_LimitAligned)
		break;
...
#endif
	m_AllocdCtrlAlignReadsLoci = cAlignReadsLociInitalAlloc;
	m_AllocdCtrlAlignReadsMem = memreq;
	m_CurNumCtrlAlignReadsLoci = 0;
...
m_MeanLenCtrlReads = (uint32_t)( m_CurSumCtrlReadsLen / m_NumLoadedCtrlReads);
...
if(m_NumLoadedExprReads)
	m_MeanLenExprReads = (uint32_t)( m_CurSumExprReadsLen / m_NumLoadedExprReads);
...
int CurAlignDiffIdx;
uint8_t CurSense;
uint32_t ControlCnts;
uint32_t ExperimentCnts;
uint32_t SumControlReadLen;
uint32_t SumExperimentReadLen;
uint32_t MeanControlReadLen;
uint32_t MeanExperimentReadLen;
int CurLoci;
int CurBin;
int PrvBin;
tsAlignReadLoci *pAlignReadLoci;

if((AlignIdx = (uint32_t)LocateStartAlignment(m_DEAlignStrand,ChromID,StartLoci,EndLoci,NumAlignReadsLoci,pAlignReadsLoci))==0)
	return(eBSFSuccess);
...
uint32_t SumFeatCtrlPoissonCnts;
uint32_t SumFeatExprPoissonCnts;
...
uint32_t TotPoissonCtrlLibCnts = 0;
uint32_t TotPoissonExprLibCnts = 0;
...
for(PermIter = 0; PermIter < MaxNumPerms; PermIter++,pPearson++,pFeatFoldChanges++,pPValues++)
	...
	*pPearson = CurPearson;
	...
	PoissonCtrlLibCnts = (uint32_t)RandPoisson(pThreadInst,m_NumNormCtrlReads  - SumFeatCtrlPoissonCnts);
...
for(Idx = 0; Idx < m_NumBins; Idx++, pAlignBin++,pCtrlBin++,pExprBin++)
	...
	if(pAlignBin->ExperimentCoverage > 0)
		...
		pAlignBin->ExperimentCoverage = (pAlignBin->ExperimentCoverage + (BinLen/2))  / BinLen; // normalise for length of bin to get depth, rounding up ...
		if(pAlignBin->ExperimentCoverage == 0)
