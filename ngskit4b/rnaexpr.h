...
class CRNAExpr
{
	...
	int32_t m_NumbHdrRNAMappings;					// number of header RNA sample identifiers mappings in m_HdrRNAMappings
	int32_t m_HdrRNAMappings[cMaxRNAESamples];		// mappings between order of header RNA sample names in RNA file and RNA sample names in materials file

	int32_t m_NumbHdrWGSMappings;					// number of header WGS sample identifier mappings in m_HdrWGSMappings
	int32_t m_HdrWGSMappings[cMaxWGSSamples];		// mappings between order of header WGS sample names in score files and WGS sample names in materials file

	int32_t m_NumbRowRNAMappings;					// number of row RNA sample identifiers mappings in m_RowRNAMappings
	int32_t m_RowRNAMappings[cMaxRNAESamples];		// mappings between order of row RNA sample names in RNA file and RNA sample names in materials file

	int32_t m_NumbRowWGSMappings;					// number of header WGS sample identifier mappings in m_RowWGSMappings
	int32_t m_RowWGSMappings[cMaxWGSSamples];		// mappings between order of row WGS sample names in score files and WGS sample names in materials file
	...
	int LoadWGSScoresFile(char* pszInWGSvWGSFile);		// parse and load WGS vs. WGS homozygosity scores

	int LoadRNAScoresFile(char* pszInRNAvWGSFile);		// parse and load RNA vs. WGS homozygosity scores
	...
	double	// returned correlation coefficient, used for finding maximal replicate pairing pearson 'r's
		PearsonCorrelation(int32_t StartFeatureID,	// starting from this feature (row)
	...
	double	// returned correlation coefficient, , used for finding maximal replicate with WGS pearson 'r's
		PearsonCorrelation(int32_t RNArowIdx,	// correlate this RNA row
