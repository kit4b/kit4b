...
for(ChkPtID = 0; ChkPtID < m_MaxChkPtReqs; ChkPtID+=1)
	...
	if(pChkPtReqs == NULL || (pChkPtReqs->SessionID == 0 && pChkPtReqs->AllocMem < ReallocTo))
		...
		if(pChkPtReqs == NULL)
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName,"CreateChkPtReqs: Memory allocation of %zd bytes failed - %s",(int64_t)ReallocTo,strerror(errno));
			return(eBSFerrMem);
			}
...
if(MemUsed >= AllocMem)
	...
	if(pReallocd == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName,"AddChkPtReq: Memory reallocation to %zd bytes failed - %s",(int64_t)ReallocTo,strerror(errno));
		return(false);
		}
...
if(RespSize >= pChkPtReqs->AllocRespData)
	...
	if(pReallocdResp == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal, gszProcName,"AddChkPtResp: Memory reallocation to %zd bytes failed - %s",(int64_t)ReallocTo,strerror(errno));
		return(false);
		}
	pChkPtReqs->AllocRespData = (uint32_t)ReallocTo;
	pChkPtReqs->pRespData = pReallocdResp;
