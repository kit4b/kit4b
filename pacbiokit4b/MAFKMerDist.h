...
const uint32_t cMaxMAFBlockRowLen = 0x03fffff;    // assuming upper limit on length of any single MAF row
const uint32_t cMaxMAFKmerDistBlockLen = (cMaxMAFBlockRowLen * 100); // expect maximally sized alignment blocks to be no m ore than this size
const uint32_t cAllocRptBuffSize = 0x07ffff;      // output report buffer size
...
typedef struct TAG_sKmerConfBase {   // associating consensus base with corresponding concensus confidence
	...
	uint8_t Conf;				// confidence in consensus base
...
class CMAFKMerDist
{
	...
	uint32_t m_NumParsedBlocks;       // this many alignment blocks have been parsed from MAF file
	...
	uint32_t m_OfsRptBuff;			// offset in m_pszRptBuff at which to next write
	uint32_t m_AllocRptBuffSize;      // m_pszRptBuff allocation size
