...
if(m_pAllocdCells == NULL || (m_AllocdCells < (MaxTargLen + 5)))  // allowing a few additional cells to reduce potential for reallocations required
	...
	if(m_pAllocdCells == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"Fatal: unable to allocate %zd bytes contiguous memory for cells",(int64_t)m_AllocdCellSize);
		m_AllocdCellSize = 0;
		m_AllocdCells = 0;
		return(false);
		}
...
if(MaxOverlapLen > 0)
	...
	if(m_pAllocdTracebacks == NULL)
		...
		if (m_pAllocdTracebacks == NULL)
			{
			gDiagnostics.DiagOut(eDLFatal, gszProcName, "Fatal: unable to allocate %zd bytes contiguous memory for traceback cells", (int64_t)m_AllocdTracebacksSize);
			m_AllocdTracebacksSize = 0;
			m_AllocdTracebacks = 0;
			return(false);
			}
	...
	if(m_pMAAlignOps == NULL)
		...
		if(m_pMAAlignOps == NULL)
			{
			gDiagnostics.DiagOut(eDLFatal,gszProcName,"Fatal: unable to allocate %zd bytes contiguous memory for multialignment operators",(int64_t)m_AllocdMAAlignOpsSize);
			m_AllocdMAAlignOpsSize = 0;
			return(false);
			}
		}
...
for(IdxP = 0; IdxP < ProbeRelLen; IdxP++)
	...
	if(m_pAllocdTracebacks != NULL && (m_UsedTracebacks + 10) > (m_AllocdTracebacks - TargRelLen)) // ensure that sufficient memory has been allocated to hold any tracebacks in next sweep over the target
		...
		if((m_UsedTracebacks + 10) > (m_AllocdTracebacks - TargRelLen))
			...
			if(pAllocd == NULL)
				{
				gDiagnostics.DiagOut(eDLFatal,gszProcName,"Align: Memory re-allocation to %zd bytes - %s",memreq,strerror(errno));
				return(NULL);
				}
...
if(m_pMACols == NULL || (memreq + 10000) > m_AllocMAColsSize)
	...
	if(m_pMACols == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"Fatal: unable to allocate %zd bytes contiguous memory for multialignment",(int64_t)memreq);
		return(eBSFerrMem);
		}
#else
		// gnu malloc is still in the 32bit world and seems to have issues if more than 2GB allocation
...
do {
	if((m_MAAlignOps + 100) > m_AllocdMAAlignOpsSize)	// realloc as may be required
		...
		if(pAllocd == NULL)
			{
			gDiagnostics.DiagOut(eDLFatal,gszProcName,"TracebacksToAlignOps: Memory re-allocation to %zd bytes - %s",memreq,strerror(errno));
			return(eBSFerrMem);
			}
...
if(m_MACols + 10 >= m_AllocMACols)			// need to extend previously allocated columns to hold this new column?
	...
	if(pAllocd == NULL)
		{
		gDiagnostics.DiagOut(eDLFatal,gszProcName,"InsertCol: Memory re-allocation to %zd bytes - %s",memreq,strerror(errno));
		return(NULL);
		}
